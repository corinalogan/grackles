...
author: '[Dr. Corina Logan](http://CorinaLogan.com) (University of Cambridge, cl417@cam.ac.uk)'
date: '`r Sys.Date()`'
output:
...
I may present the results from different hypotheses in separate papers.

###C. HYPOTHESES

####H1: Behavioral flexibility is positively correlated with exploration and neophilia, but not other behaviors.

**Predictions 1-5:** Individuals in the experimental group where flexibility (as measured by reversal learning and on a multi-access box) was manipulated (and increased) will be more exploratory (P1; methods as in @mettke2009spatial) and more neophilic (P2) than individuals in the control group where flexibility was not increased, and there will be no difference between the groups in persistence (P3), neophobia (P4, methods as in @logan2016behavioral), or motor diversity (P5) (as found in @logan2016behavioral). 
...
There is no randomizing or counterbalancing in these comparisons.
...
2) Persistence: Number of touches to an apparatus per time (multi-access box in a separate preregistration, novel sub-environment in P1, novel objects in P2 and P4)
...
6) Number of touches to the non-functional part of an apparatus per time (multi-access box) 
...
8) Number of different motor actions used when attempting to solve the multi-access box 
...
11) Condition: control, flexibility manipulation

*P3: does persistence correlate with reversal persistence?*

1) Average number of touches to the functional part of an apparatus per time (multi-access box, novel sub-environment in P1, novel objects in P2 and P4)
...
I do not plan to **exclude** any data. When **missing data** occur, the existing data for that individual will be included in the analyses for the tests they completed.
...
#Check dependent variable: number of trials to reverse last preference
last <- read.csv ("/Users/corina/R/GrackleTestsYear1/Exploration/reverselatency.csv", header=T, sep=",", stringsAsFactors=F) 
...
Y2 <- rnorm(1281, mean=mean(last$TrialsToReverseLast), sd=sd(last$TrialsToReverseLast))
hist(Y2, xlab="Number of trials to reverse the last preference", main="Simulated Data")
```

```{r dist_check2, eval=FALSE, warning=FALSE, results='asis', echo=TRUE, fig.cap="**Figure 3.** Visual check of the Normal Q-Q plot to determine whether the residuals are normally distributed."}
#Check dependent variable: number of trials to reverse last preference
last <- read.csv ("/Users/corina/R/GrackleTestsYear1/Exploration/reverselatency.csv", header=T, sep=",", stringsAsFactors=F) 
...
```{r dist_check3, eval=FALSE, warning=FALSE, results='asis', echo=TRUE, fig.cap="**Figure 4.** The distribution of the actual data versus what a normal distribution would look like with simulated data."}
#Check dependent variable: number of incorrect trials in the final reversal before making the correct choice
incorrect <- read.csv ("/Users/corina/R/GrackleTestsYear1/Exploration/reverselatency.csv", header=T, sep=",", stringsAsFactors=F) 

op <- par(mfrow=c(1,2), mar=c(4,4,2,0.2))
#Figure A-B. This is what the distribution of actual data looks like
hist(incorrect$NumberIncorrectTrialsReversal, xlab="Number of 'no choice' trials in the first reversal", main="Actual Data")
#Figure C-D. Given the actual data, this is what a normal distribution would look like
Y2 <- rnorm(1281, mean=mean(incorrect$NumberIncorrectTrialsReversal), sd=sd(incorrect$NumberIncorrectTrialsReversal))
hist(Y2, xlab="Number of 'no choice' trials in the first reversal", main="Simulated Data")
```

```{r dist_check4, eval=FALSE, warning=FALSE, results='asis', echo=TRUE, fig.cap="**Figure 5.** Visual check of the Normal Q-Q plot to determine whether the residuals are normally distributed."}
#Check dependent variable: number of incorrect trials in the final reversal before making the correct choice
incorrect <- read.csv ("/Users/corina/R/GrackleTestsYear1/Exploration/reverselatency.csv", header=T, sep=",", stringsAsFactors=F) 

#Figure 2. Visual check of the Normal Q-Q plot
op <- par(mfrow=c(1,4), mar=c(4,4,2,0.2))
plot(glm(incorrect$NumberIncorrectTrialsReversal~incorrect$LatencyNovelEnvExploration)) 
```

```{r dist_check5, eval=FALSE, warning=FALSE, results='asis', echo=TRUE, fig.cap="**Figure 6.** The distribution of the actual data versus what a normal distribution would look like with simulated data."}
#Check dependent variable: Neophobia - latency to land on the table
neophobia <- read.csv ("/Users/corina/R/GrackleTestsYear1/Exploration/neophobia.csv", header=T, sep=",", stringsAsFactors=F) 
...
hist(Y2, xlab="Latency to land on table", main="Simulated Data")
```

```{r dist_check6, eval=FALSE, warning=FALSE, results='asis', echo=TRUE, fig.cap="**Figure 7.** Visual check of the Normal Q-Q plot to determine whether the residuals are normally distributed."}
#Check dependent variable: Neophobia - latency to land on the table
neophobia <- read.csv ("/Users/corina/R/GrackleTestsYear1/Exploration/neophobia.csv", header=T, sep=",", stringsAsFactors=F) 
...
```{r dist_check7, eval=FALSE, warning=FALSE, results='asis', echo=TRUE, fig.cap="**Figure 8.** The distribution of the actual data versus what a normal distribution would look like with simulated data."}
#Check dependent variable: Persistence - number of touches to an apparatus per time
neophobia <- read.csv ("/Users/corina/R/GrackleTestsYear1/Exploration/neophobia.csv", header=T, sep=",", stringsAsFactors=F) 
...
#Figure C-D. Given the actual data, this is what a normal distribution would look like
Y2 <- rnorm(1281, mean=mean(neophobia$NoObjectTouchesPerTime), sd=sd(neophobia$NoObjectTouchesPerTime))
hist(Y2, xlab="Number of touches to an apparatus per time", main="Simulated Data")
```

```{r dist_check8, eval=FALSE, warning=FALSE, results='asis', echo=TRUE, fig.cap="**Figure 9.** Visual check of the Normal Q-Q plot to determine whether the residuals are normally distributed."}
#Check dependent variable: Persistence - number of touches to an apparatus per time
neophobia <- read.csv ("/Users/corina/R/GrackleTestsYear1/Exploration/neophobia.csv", header=T, sep=",", stringsAsFactors=F) 
...
explore <- read.csv ("/Users/corina/R/GrackleTestsYear1/Exploration/explore.csv", header=T, sep=",", stringsAsFactors=F) 
...
persist2 <- read.csv ("/Users/corina/R/GrackleTestsYear1/Exploration/reverselatency.csv", header=T, sep=",", stringsAsFactors=F) 
...
#Neophobia
neo <- read.csv ("/Users/corina/R/GrackleTestsYear1/Exploration/neophobia.csv", header=T, sep=",", stringsAsFactors=F) 
...
#Persistence
per <- read.csv ("/Users/corina/R/GrackleTestsYear1/Exploration/persist.csv", header=T, sep=",", stringsAsFactors=F) 
