...
Individuals show consistent individual differences in behavior if the variance in latency to approach the task is smaller within individuals compared to variance in latency among individuals (for exploration and boldness assays). The same definition applies to persistence with the number of touches as the dependent variable, and to motor diversity with the number of different motor actions as the dependent variable.
...
1) Boldness: Latency to land on the table - OR - Latency to eat the food - OR - Latency to touch an object next to food (we will choose the variable with the most data)
