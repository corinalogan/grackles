...
Repeatability is indicated if the variance in latency is smaller within individuals compared to variance in latency among individuals (or something that mentions variance partitioning instead of correlations). The same definition is used for the boldness test which is measured by the latency to land on a table next to a novel object. 
...
1) Boldness: Latency to land on the table - OR - Latency to eat the food - OR - Latency to touch a novel object that is next to food (I will choose the variable with the most data)
...
1) Day (boldness) or test (persistence)
...
#Boldness
neo <- read.csv ("/Users/corina/GTGR/data/data_boldness.csv", header=T, sep=",", stringsAsFactors=F) 
