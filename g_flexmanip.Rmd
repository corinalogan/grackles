---
title: Is behavioral flexibility manipulatable and, if so, does it improve flexibility
  and problem solving in a new context?
...
This preregistration was written prior to collecting data.

###B. PARTITIONING THE RESULTS

I may present the different hypotheses in separate papers.

###C. HYPOTHESES

#### H1: Behavioral flexibility, as measured by reversal learning, is manipulatable.

**Prediction 1:** Individuals improve their flexibility on a serial reversal learning task by requiring fewer trials to reverse a preference with each successive reversal. 
...
####*P1: reversal speed gets faster with serial*
...
#Trials to reverse
reverse <- read.csv ("/Users/corina/R/GrackleTestsYear1/FlexibilityManipulation/reverse.csv", header=T, sep=",", stringsAsFactors=F) 
...
Y2 <- rnorm(1281, mean=mean(reverse$TrialsToReverse), sd=sd(reverse$TrialsToReverse))
hist(Y2, xlab="Number of trials to reverse a preference", main="Simulated Data")
```
...
```{r dist_check3, eval=FALSE, warning=FALSE, results='asis', echo=TRUE, fig.cap="**Figure 3.** The distribution of the actual data versus what a normal distribution would look like with simulated data."}
#Latency to solve a new loci
diversity <- read.csv ("/Users/corina/R/GrackleTestsYear1/FlexibilityManipulation/reversemulti.csv", header=T, sep=",", stringsAsFactors=F) 

op <- par(mfrow=c(1,2), mar=c(4,4,2,0.2))
#Figure A-B. This is what the distribution of actual data looks like
hist(diversity$AvgLatencySolveNewLoci, xlab="Latency to solve a new loci (s)", main="Actual Data")
#Figure C-D. Given the actual data, this is what a normal distribution would look like
Y2 <- rnorm(1281, mean=mean(diversity$AvgLatencySolveNewLoci), sd=sd(diversity$AvgLatencySolveNewLoci))
hist(Y2, xlab="Latency to solve a new loci (s)", main="Simulated Data")
```

```{r dist_check4, eval=FALSE, warning=FALSE, results='asis', echo=TRUE, fig.cap="**Figure 4.** Visual check of the Normal Q-Q plot to determine whether the residuals are normally distributed."}
#Latency to solve a new loci
diversity <- read.csv ("/Users/corina/R/GrackleTestsYear1/FlexibilityManipulation/reversemulti.csv", header=T, sep=",", stringsAsFactors=F) 
...
```{r dist_check5, eval=FALSE, warning=FALSE, results='asis', echo=TRUE, fig.cap="**Figure 3.** The distribution of the actual data versus what a normal distribution would look like with simulated data."}
#Latency to attempt to solve a new loci
diversity <- read.csv ("/Users/corina/R/GrackleTestsYear1/FlexibilityManipulation/reversemulti.csv", header=T, sep=",", stringsAsFactors=F) 

op <- par(mfrow=c(1,2), mar=c(4,4,2,0.2))
#Figure A-B. This is what the distribution of actual data looks like
hist(diversity$AvgLatencyAttemptNewLoci, xlab="Latency to solve a new loci (s)", main="Actual Data")
#Figure C-D. Given the actual data, this is what a normal distribution would look like
...
serial <- read.csv ("/Users/corina/R/GrackleTestsYear1/FlexibilityManipulation/reverse.csv", header=T, sep=",", stringsAsFactors=F) 
...
improve <- read.csv ("/Users/corina/R/GrackleTestsYear1/FlexibilityManipulation/reversemulti.csv", header=T, sep=",", stringsAsFactors=F) 
...
serial2 <- read.csv ("/Users/corina/R/GrackleTestsYear1/FlexibilityManipulation/reverse.csv", header=T, sep=",", stringsAsFactors=F) 
...
improve1 <- read.csv ("/Users/corina/R/GrackleTestsYear1/FlexibilityManipulation/reversemulti.csv", header=T, sep=",", stringsAsFactors=F) 

#GLM: color/shape reversal vs multi-access box
rm <- glm(AvgTrialsToReverse ~ AvgLatencyNewLoci, data=improve1)
summary(rm)
```

```{r consistent2, eval=FALSE, warning=FALSE, results='asis', echo=TRUE, include=TRUE}
improve2 <- read.csv ("/Users/corina/R/GrackleTestsYear1/FlexibilityManipulation/reversetouch.csv", header=T, sep=",", stringsAsFactors=F) 
...
I will test as many birds as I can in the two years I have at this field site given that the birds only participate in tests in aviaries during the non-breeding season (September through February). The minimum sample size will be 8 birds per experiment (n=16 total), however I expect to be able to test many more.
