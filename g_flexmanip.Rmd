...
Output:		Noncentrality parameter λ     	=	6,7200000
...
To determine our ability to detect actual effects, we ran power analyses in G&ast;Power with the following settings: test family=F tests, statistical test=linear multiple regression: Fixed model (R^2 deviation from zero), type of power analysis=a priori, alpha error probability=0.05. We reduced the power to 0.70 and increased the effect size until the total sample size in the output matched our projected sample size (n=32). Note that only the fixed effect variables were entered as the number of predictors and not the random effect variables because it does not appear possible to run a power analysis on a mixed model in G&ast;Power. The protocol of the power analysis is here:
