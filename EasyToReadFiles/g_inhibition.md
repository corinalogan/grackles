...
**Prior to collecting any data:** This preregistration was written (2017).
...
**P2:** If go no-go task performance strongly correlates with performance on the delayed gratification task, both of which measure the same type of inhibition, this indicates these two tasks measure the same trait, which therefore validates an inhibition task using a touch screen (the go no-go task). Note that the detour task measures a different type of inhibition (and it is questionable whether it actually measures inhibition; e.g., Van Horik et al. 2018), which is why we are validating the go no-go task against the delayed gratification task. If the detour results correlate with performances on the other two inhibition tasks, this would provide some evidence that the detour task is a measure of inhibition as is claimed.
