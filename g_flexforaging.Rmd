...
**P5** Flexible individuals are more likely to have stronger bonds with others, in particular with individuals who are less related, potentially because they are better able to adjust their behavior to that of an affiliate. Bond strength is measured using the focal follow method to sample affiliative and aggressive behaviors.

**P5 alternative 1:** Flexible individuals are not more likely to have stronger bonds with each other, potentially because all individuals are able to form bonds with like individuals, including the less flexible individuals.

**P5 alternative 2:** Flexible individuals are less likely to have stronger bonds with each other, potentially because they frequently change their behavior and are difficult to form bonds with.

#### H3: Individuals that are more [behaviorally flexible](./g_flexmanip.Rmd) (measured by reversal learning and switching between options on a multi-access box) will use a wider range of habitats.

**P6:** Individuals immigrating into a population are more likely to be flexible, potentially because they need to learn how to obtain resources in an unfamiliar area. Immigrants are individuals who carry many genetic variants (identified using ddRADseq) that are not found in other individuals in this population.

**P6 alternative:** Individuals immigrating into a population are not more likely to be flexible, potentially because the human urban environment is comparable across landscapes. 

**P7:** Flexible individuals will be found more regularly in a wider diversity of habitats.

**P7 alternative:** Flexibility is not associated with presence in diverse habitats because the more flexible individuals might specialize in specific foraging strategies. 
...
6) Population: center (Central America), middle (Arizona), edge (northern US)

***P3: flexible = more human foods***

1) Proportion of diet per individual that is human food

2) Flexibility manipulation: control, flexibility manipulation

3) Dominance rank

4) ID

5) Population: center (Central America), middle (Arizona), edge (northern US)

***P4: flexible = disperse***
...
#GLMM with response variable = trials to reverse the last preference
#females
f1 <- MCMCglmm(TrialsToReverseLast ~ ProportionFoodsEatenHumanFood + ExperimentalGroup + DominanceRank, random=~Population+ID, family="poisson", data=fem, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f1)
#autocorr(f1$Sol) #Did fixed effects converge?
#autocorr(f1$VCV) #Did random effects converge?

#males
f2 <- MCMCglmm(TrialsToReverseLast ~ ProportionFoodsEatenHumanFood + ExperimentalGroup + DominanceRank, random=~Population+ID, family="poisson", data=mal, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f2)
#autocorr(f2$Sol) #Did fixed effects converge?
#autocorr(f2$VCV) #Did random effects converge?
...
#GLMM with response variable = flexibility ratio
#female
f3 <- MCMCglmm(TrialsToReverseLast ~ ProportionFoodsEatenHumanFood + ExperimentalGroup + DominanceRank, random=~Population+ID, family="poisson", data=fem, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f3)
#autocorr(f3$Sol) #Did fixed effects converge?
#autocorr(f3$VCV) #Did random effects converge?

#male
f4 <- MCMCglmm(TrialsToReverseLast ~ ProportionFoodsEatenHumanFood + ExperimentalGroup + DominanceRank, random=~Population+ID, family="poisson", data=mal, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f4)
#autocorr(f4$Sol) #Did fixed effects converge?
#autocorr(f4$VCV) #Did random effects converge?
```
...
An individual's average relatedness to everyone within a certain geographic range
...
#GLMM with response variable = trials to reverse the last preference
#females
f1 <- MCMCglmm(TrialsToReverseLast ~ Immigrant, random=~Population+ID, family="poisson", data=fem, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f1)
#autocorr(f1$Sol) #Did fixed effects converge?
#autocorr(f1$VCV) #Did random effects converge?

#males
f2 <- MCMCglmm(TrialsToReverseLast ~ Immigrant, random=~Population+ID, family="poisson", data=mal, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f2)
#autocorr(f2$Sol) #Did fixed effects converge?
#autocorr(f2$VCV) #Did random effects converge?
...
#GLMM with response variable = flexibility ratio
#female
f3 <- MCMCglmm(TrialsToReverseLast ~ Immigrant, random=~Population+ID, family="poisson", data=fem, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f3)
#autocorr(f3$Sol) #Did fixed effects converge?
#autocorr(f3$VCV) #Did random effects converge?

#male
f4 <- MCMCglmm(TrialsToReverseLast ~ Immigrant, random=~Population+ID, family="poisson", data=mal, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f4)
#autocorr(f4$Sol) #Did fixed effects converge?
#autocorr(f4$VCV) #Did random effects converge?
```
...
#GLMM with response variable = trials to reverse the last preference
#females
f1 <- MCMCglmm(TrialsToReverseLast ~ MaxBondStrength + AvgBondStrength, random=~Population+ID, family="poisson", data=fem, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f1)
#autocorr(f1$Sol) #Did fixed effects converge?
#autocorr(f1$VCV) #Did random effects converge?

#males
f2 <- MCMCglmm(TrialsToReverseLast ~ MaxBondStrength + AvgBondStrength + MaleSharesTerritory, random=~Population+ID, family="poisson", data=mal, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f2)
#autocorr(f2$Sol) #Did fixed effects converge?
#autocorr(f2$VCV) #Did random effects converge?
...
#GLMM with response variable = flexibility ratio
#female
f3 <- MCMCglmm(TrialsToReverseLast ~ MaxBondStrength + AvgBondStrength, random=~Population+ID, family="poisson", data=fem, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f3)
#autocorr(f3$Sol) #Did fixed effects converge?
#autocorr(f3$VCV) #Did random effects converge?

#male
f4 <- MCMCglmm(TrialsToReverseLast ~ MaxBondStrength + AvgBondStrength + MaleSharesTerritory, random=~Population+ID, family="poisson", data=mal, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f4)
#autocorr(f4$Sol) #Did fixed effects converge?
#autocorr(f4$VCV) #Did random effects converge?
```
