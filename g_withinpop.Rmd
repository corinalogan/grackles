...
The way we define flexibility (individuals change behavior according to changing circumstances using learning from previous experiences) assumes that being flexible will always be associated with the higher fitness payoff (e.g., higher quality food rewards). Therefore, the reasons individuals might not be flexible are 1) there are indirect costs to flexibilty (e.g., exploration, risks involved in uncertain choices), 2) individuals live in environments that rarely change or change randomly, or 3) individuals have internal constraints (e.g., not enough energy). Points [1](./g_exploration.Rmd) and [2](./g_expansion.Rmd) are being tested in different preregistrations. Here we test point 3.
...
**Prediction 1:** Individuals with higher glucocorticoid concentrations at the time of capture will perform more poorly in causal inference experiments (see [causal cognition preregistration](./g_inhibition.Rmd)), solve fewer loci and switch between loci slower on the multi-access box, and require more trials to reverse preferences (see [flexibility preregistration](./g_flexmanip.Rmd)).

**P1 alternative 1:** If flexibility positively correlates with glucocorticoid concentrations at the time of capture, this could be due to 1) the more dominant individuals incurring higher stress levels to maintain dominance, if the more dominant individuals are also more flexible; or 2) the more flexible individuals access higher quality resources and thus have more energy to mount a stress response.

**P1 alternative 2:** If flexibility and cognitive performance do not correlate with glucocorticoid concentrations at the time of capture, this indicates these traits are independent from each other or that they are related to another variable that was not measured.
...
#### H3.4: There will be a positive correlation between neophila and markers of basal immunity
...
#### H5: Individuals that are more behaviorally flexible have lower parasite loads (and thus more available energy).

**P8:** The more flexible individuals (on the multi-access box and in reversal learning) will have fewer haemosporidian parasite counts and lower parasite species diversity.

**P8 alternative 1:** If there is a positive relationship between flexibility and parasite loads, this could be because the more flexible individuals eat a wider range of foods and are thus exposed to more parasites.
...
#### H6: Individuals that are more behaviorally flexible have microbiomes with higher species diversity, which could facilitate the digestion of diverse foods and the adaptation to new parasites/bacteria.
...
**P9 alternative 1:** There is no difference in the number of species in the grackle microbiome in relation to flexibility, potentially because there is little between individual variation in the number of microbiome species each grackle hosts.
...
1) P1 stress: glucocorticoid concentration
...
14) P8 parasites: number of parasites per sample

15) P8 parasites: number of parasites species per sample

16) P8 parasites: parasite species diversity per sample (***how to caclculate?***)

17) P9 microbiome: microbiome species diversity per sample (***how to caclculate?***)

18) Population (as a random effect to examine within population variation)
...
1) Parasites: number of parasites per sample

2) Parasites: number of parasites species per sample

3) Parasites: parasite species diversity per sample
...
#GLMM
library(MCMCglmm)
prior = list(R=list(R1=list(V=1,nu=0),R2=list(V=1,nu=0),R3=list(V=1,nu=0),R4=list(V=1,nu=0),R5=list(V=1,nu=0),R6=list(V=1,nu=0),R7=list(V=1,nu=0),R8=list(V=1,nu=0),R9=list(V=1,nu=0),R10=list(V=1,nu=0)), G=list(G1=list(V=1,nu=0),G2=list(V=1,nu=0)))
...
#females
f1 <- MCMCglmm(TrialsToReverseLast ~ Cort + NumberHeterophil + NumberLymphocytes + InflammatoryGene + NumberLeucocytes + AntiInflammatoryGene + Th2Gene + NumberParasites + ParasiteSpecies + ParasiteDiversity, random=~Population+AvgTarsusLength, family="poisson", data=fem, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f1)
#autocorr(f1$Sol) #Did fixed effects converge?
#autocorr(f1$VCV) #Did random effects converge?

#males
m1 <- MCMCglmm(TrialsToReverseLast ~ Cort + NumberHeterophil + NumberLymphocytes + InflammatoryGene + NumberLeucocytes + AntiInflammatoryGene + Th2Gene + NumberParasites + ParasiteSpecies + ParasiteDiversity, random=~Population+AvgTarsusLength, family="poisson", data=mal, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(m1)
...
#females
f2 <- MCMCglmm(TrialsToReverseLast ~ Cort + NumberHeterophil + NumberLymphocytes + InflammatoryGene + NumberLeucocytes + AntiInflammatoryGene + Th2Gene + NumberParasites + ParasiteSpecies + ParasiteDiversity, random=~Population+AvgTarsusLength, family="poisson", data=fem, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f2)
#autocorr(f2$Sol) #Did fixed effects converge?
#autocorr(f2$VCV) #Did random effects converge?

#males
m2 <- MCMCglmm(TrialsToReverseLast ~ Cort + NumberHeterophil + NumberLymphocytes + InflammatoryGene + NumberLeucocytes + AntiInflammatoryGene + Th2Gene + NumberParasites + ParasiteSpecies + ParasiteDiversity, random=~Population+AvgTarsusLength, family="poisson", data=mal, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(m2)
...
#GLMM with response variable = trials to reverse the last preference
#females
f1 <- MCMCglmm(TrialsToReverseLast ~ Testosterone + Hematocrit + NumberRedBloodCells + Weight + AvgTarsusLength + AvgTailLength + AvgWingLength + MicrobiomeDiversity, random=~Population, family="poisson", data=fem, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f1)
#autocorr(f1$Sol) #Did fixed effects converge?
#autocorr(f1$VCV) #Did random effects converge?

#males
m1 <- MCMCglmm(TrialsToReverseLast ~ Testosterone + Hematocrit + NumberRedBloodCells + Weight + AvgTarsusLength + AvgTailLength + AvgWingLength + MicrobiomeDiversity, random=~Population, family="poisson", data=mal, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(m1)
#autocorr(m1$Sol) #Did fixed effects converge?
#autocorr(m1$VCV) #Did random effects converge?
...
#GLMM with response variable = flexibility ratio
#females
f2 <- MCMCglmm(FlexRatio ~ Testosterone + Hematocrit + NumberRedBloodCells + Weight + AvgTarsusLength + AvgTailLength + AvgWingLength + MicrobiomeDiversity, random=~Population, family="poisson", data=fem, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f2)
#autocorr(f2$Sol) #Did fixed effects converge?
#autocorr(f2$VCV) #Did random effects converge?

#males
m2 <- MCMCglmm(FlexRatio ~ Testosterone + Hematocrit + NumberRedBloodCells + Weight + AvgTarsusLength + AvgTailLength + AvgWingLength + MicrobiomeDiversity, random=~Population, family="poisson", data=mal, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(m2)
#autocorr(m2$Sol) #Did fixed effects converge?
#autocorr(m2$VCV) #Did random effects converge?
```
...
#GLMM
library(MCMCglmm)
prior = list(R=list(R1=list(V=1,nu=0),R2=list(V=1,nu=0),R3=list(V=1,nu=0),R4=list(V=1,nu=0),R5=list(V=1,nu=0)), G=list(G1=list(V=1,nu=0)))
...
#GLMM
#females
f1 <- MCMCglmm(NumberFoodsEaten ~ NumberParasites + ParasiteSpecies + ParasiteDiversity + TrialsToReverseLast + FlexRatio, random=~Population, family="poisson", data=fem, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(f1)
#autocorr(f1$Sol) #Did fixed effects converge?
#autocorr(f1$VCV) #Did random effects converge?

#males
m1 <- MCMCglmm(NumberFoodsEaten ~ NumberParasites + ParasiteSpecies + ParasiteDiversity + TrialsToReverseLast + FlexRatio, random=~Population, family="poisson", data=mal, verbose=F, prior=prior, nitt=130000, thin=10, burnin=30000)
summary(m1)
#autocorr(m1$Sol) #Did fixed effects converge?
#autocorr(m1$VCV) #Did random effects converge?
```
...
plot(fem$ID,fem$MicrobiomeDiversity, xlab="Grackle ID (females)", ylab="Number of species in the microbiome") 
plot(mal$ID,mal$MicrobiomeDiversity, xlab="Grackle ID (males)", ylab="Number of species in the microbiome")
```
...
**Trumble (testosterone and glucocortocoids):** Hypothesis development, materials/lab space, sample processing, data analysis and interpretation, write up, revising/editing.
