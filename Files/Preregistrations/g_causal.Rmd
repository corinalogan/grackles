...
4) Hypotheses > Predictions: we corrected the typo that we were going to "lower latencies to successfully solve new tasks after previously solved tasks become unavailable on a multiaccess box" by changing "successfully" to "attempt to solve", which makes it consistent with our Analysis Plan. The former measures innovation, the latter measures flexibility and we were interested in the latter. As such, we also removed that we were comparing causal inference with "innovation" in Alternative 3.
