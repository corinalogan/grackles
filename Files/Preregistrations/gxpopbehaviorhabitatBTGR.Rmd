...
#load data sheets
ft <- read.csv(url("https://raw.githubusercontent.com/corinalogan/grackles/master/Files/Preregistrations/gxpopbehaviorhabitatBTGR_data_summaryInProgress.csv"), header=T, sep=",", stringsAsFactors=F)
ft <- data.frame(ft)
...
su <- read.csv(url("https://raw.githubusercontent.com/corinalogan/grackles/master/Files/Preregistrations/gxpopbehaviorhabitatBTGR_data_summaryInProgress.csv"), header=T, sep=",", stringsAsFactors=F)
su <- data.frame(su)
...
#5.4 BTGR (range: 2-11) (from data_xpop_btgr_feedersandfocals google sheet, tab: BirdSummary) 
#number focals per bird
af <- fo %>% group_by(BirdID,Site) %>% reframe(focalsperbird=unique(FocalID))
#sum number focals per bird
af1 <- af %>% group_by(BirdID,Site) %>% summarise (sumfocalsperbird=n())
af2 <- af1 %>% group_by(Site) %>% summarise (sumfocalsperbirdpersite=mean()) #NOT WORKING YET
