...
# GTGR AZ
#gt$What <- as.factor(gt$What)
#gt[gt$Population=="AZ",]$What
#length(unique(gt[gt$Population=="AZ",]$What)) #20
#unique(gt[gt$Population=="AZ",]$What) #fry, lizard, unknown, grains, insect, rock, cat food, worm, seed, food crumbs, vegetation,  fruit, bird poop, candy, vomit, misc. trash, soil, condiment, carcass, chicken. The CA pop didn't have lizard, worm, bird poop, candy, vomit, condiment, or carcass

# GTGR CA
#gt[gt$Population=="CA",]$What
#length(unique(gt[gt$Population=="CA",]$What)) #15
#unique(gt[gt$Population=="CA",]$What) #unknown, insect, grains, cat food, fry, soil,  rock, *peanut,  chicken,  vegetation, *mulch, food crumbs, misc. trash, seed, fruit. The AZ pop didn't have peanut or mulch

# BTGR
#wt$FoodType <- as.factor(wt$FoodType)
#length(unique(wt$FoodType)) #14
#unique(wt$FoodType) #Insect,  *Cheese, Grains,  Worm, Unknown, Seed, Food crumbs, *Oil, Vegetation, Fruit, Fry, Peanut, Chicken. BTGR had cheese and oil, while GTGR didn't

# COUNT food for BTGR for summary data sheet
#exclude NAs
wt$FoodType <- as.factor(wt$FoodType)
...
# COUNT number of food types per BTGR bird (already have this data for GTGR from flexforaging)
nt <- fo %>% group_by(BirdID) %>% summarise (Count=unique(FoodType))
nt1 <- data.frame(nt) #sum BTGR by hand and enter into summary data sheet: NumFoodTypes. THEN calculate FoodTypesPerSec by NumFoodTypes/SumFocalTime

# COUNT total number of food events per BTGR bird (already have this data for GTGR from flexforaging)
ne <- fo %>% group_by(BirdID) %>% summarise (sum=n())
ne1 <- data.frame(ne) #enter BTGR sums into summary data sheet: TotalFoodEvents


# COUNT food type switches per bird across all of its follows
fo$FoodSwitch<-NA
fo[1,]$FoodSwitch<-0
for(i in 2:nrow(fo)){
...
#Need the focal duration ONLY for those focals where a switch occurred. This will equalize the difference between the species where the GTGR were focalled for more than just foraging behavior, whereas the BTGR were only focalled for their foraging behavior (a focal couldn't start until the BTGR started foraging)
s1 <- fo[!(is.na(fo$FoodSwitch) | fo$FoodSwitch==""), ] #remove NAs so only the focals with switches remain
#length(s1$FoodSwitch) #1177
s2 <- s1[s1$FoodSwitch > 0,]
#length(s2$FoodSwitch) #306

fo2 <- s2 %>% group_by(BirdID,FocalID) %>% summarise(foodswitchesperfocal=sum(FoodSwitch),timeperfocal=mean(TimeInViewThisFocalSec))
fo3 <- fo2 %>% group_by(BirdID) %>% summarise(NumFoodSwitches=sum(foodswitchesperfocal),SumFocalTimeWSwitch=sum(timeperfocal))
fo3 <- data.frame(fo3) 

#add NumFoodSwitches and SumFocalTimeWSwitch columny to data_summary
f1 <- full_join(fo3,ft,by="BirdID")
write.csv(f1,"gxpopbehaviorhabitatBTGR_data_summarytest.csv") # it adds all of the birds with NAs to the sheet, how to fix this?


#Calculate food switches/bird/focal
fo2 <- fo %>% group_by(BirdID,FocalID) %>% summarise(foodswitchesperfocal=sum(FoodSwitch))  
fo2 <- data.frame(fo2)

#Calculate avg focals/bird/site
fo2.1 <- fo %>% group_by(Site,BirdID) %>% summarise(sum=length(FocalID))  #not working yet
fo2.1 <- data.frame(fo2.1)

#Calculate total switches per bird and enter this data at the summary data sheet: NumFoodSwitches
fo3 <- fo2 %>% group_by(BirdID) %>% summarise(totalfoodswitches=sum(foodswitchesperfocal)) 
fo3 <- data.frame(fo3)

# What is the average number of focals per bird? 5.4 BTGR (range: 2-11) (from data_xpop_btgr_feedersandfocals google sheet, tab: BirdSummary) 
...
# remove NAs - make sure the GTGR who did not eat food get excluded
ft1 <- ft[!(is.na(ft$NumFoodSwitches) | ft$NumFoodSwitches==""), ] #9 NAs so 9 birds were removed

# Sample size
ft1$Site <- as.factor(ft1$Site)
#length(ft1[ft1$Site=="AZ",]$Site) #13 GTGR AZ
#length(ft1[ft1$Site=="CA",]$Site) #13 GTGR CA
#length(ft1[ft1$Site=="BTGR",]$Site) #8 BTGR

# Run model
library(rethinking)
library(dplyr)
dat <- list(foodswitches = ft1$NumFoodSwitches,
            totaltime = ft1$SumFocalTime, 
            site = as.integer(as.factor(ft1$Site)) #Site 1=AZ, 2=BTGR, 3=CA
              )
switch <- ulam( alist(
        foodswitches ~ dbinom(totaltime,p),
        logit(p) <- a[site], #the intercept is the probability of observing any of the types is different between groups, but this probability doesn't change over time (bc birds are observed only AFTER the manipulation is over). Include total observation time because this varied per focal (with a target max time of 10 min)
        a[site] ~ dnorm(0,1) #each treatment gets its own intercept, mean=0 bc assume an average bird might show half of the total techs, so 0 on inverse logit = 50%
...
# a[1] -6.74 0.12 -6.93 -6.55 1.01  2111.00
# a[2] -6.38 0.14 -6.61 -6.15 1.00  2017.36
# a[3] -6.71 0.13 -6.92 -6.49 1.00  2050.76

#means at each site on the regular scale
#inv_logit(output_switch[,1]) #0.001184102 0.001700368 0.001223125
...
#        |0.89         0.89| 
#-9.76720e-04 to -8.41181e-05 
#does not cross zero so the groups differ: BTGR have higher probability of switching than GTGR AZ
#mean(diff) # -0.0005246724 per sec
#sd(diff) # 0.0002791676 per sec
#mean(diff)*60 # -0.03148034 per min

# How much more likely are the BTGR to switch than GTGR AZ?
#mean(inv_logit(post$a[,2]))/mean(inv_logit(post$a[,1])) # 1.440121
#sd(inv_logit(post$a[,2]))/mean(inv_logit(post$a[,1])) # 0.2002083
...
#        |0.89         0.89| 
#-0.0009137613  0.0000231731
#crosses zero so  groups do not differ: GTGR CA does not have higher probability of switching than BTGR
#mean(diffcb) #-0.0004826716 per sec
#sd(diffcb) # 0.0002938304 per sec
#mean(diffcb)*60 # -0.0289603 per min

# How much more likely are the GTGR CA to switch than BTGR?
#mean(inv_logit(post$a[,2]))/mean(inv_logit(post$a[,3])) # 1.391109
#sd(inv_logit(post$a[,2]))/mean(inv_logit(post$a[,3])) # 0.1933946
...
#        |0.89         0.89| 
#-0.0003572783  0.0003231871 
#crosses zero so the groups di not differ: GTGR AZ and GTGR CA have similar probability of switching
#mean(diffca) # -4.200077e-05 per sec
#sd(diffca) # 0.0002145229 per sec
#mean(diffca)*60 # -0.002520046 per min

# How much more likely are the GTGR AZ to switch than GTGR CA?
#mean(inv_logit(post$a[,3]))/mean(inv_logit(post$a[,1])) # 1.035232
#sd(inv_logit(post$a[,3]))/mean(inv_logit(post$a[,1])) # 0.1382212
...
ft1$probswitch <- (ft1$NumFoodSwitches/ft1$SumFocalTime)*60 #Ak'xi in CA has a really high rate

op <- par(mfrow=c(1,1), mar=c(3,4.9,2,0.9))
plot( as.integer(as.factor(ft1$Site))+rnorm(34,0,0.1) , ft1$probswitch , xlab="" , ylab="Probability of switching among food types / min" , ylim=c(0,1.3), xlim=c(0.6,3.5), xaxt="n")
#AZ
