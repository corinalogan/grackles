...
P6: Grackles will show repeatable performance on the learning task over sequential trials with two functionally similar but visually distinct apparatuses (@logan2016new, @mccune2019captive). This would indicate that behavioral interactions with the task result from inherent learning abilities.
...
Great-tailed grackles are caught in the wild in Tempe, Arizona USA for individual identification (colored leg bands in unique combinations) until as much of the population is banded as possible (estimated 400 grackles in the area, 56 have been banded as of 6 May 2019). Some individuals (~6-16) will be brought temporarily into aviaries where demonstrators will be trained on a particular locus of the apparatus before being released back to the wild to serve as demonstrators for the experiment, which will occur in the wild (in the same way as @logan2016new, except in the wild). We will attempt to train grackles in the aviaries on one solving method per bird on per apparatus for the two visually distinct but functionally similar apparatuses (@logan2016new, @mccune2019captive). However, if the solving methods on one apparatus are too difficult for the aviary grackles to learn quickly, then we will only use easier apparatus for the experiment in the wild and we will not test H3. 
...
In the population in the middle of the expanded range, we will attempt to counterbalance demonstrators among the three groups such that each group will observe a demonstrator that was trained in a distinct solving method, and observers in each group will not be exposed to other demonstrators using different opening methods. If we are able to train demonstrators on solving methods for both of the apparatuses, then we will counterbalance which group sees which apparatus type first. We will counterbalance the core and edge populations by training demonstrators on one solving method in each group (one group per population) such that the core population demonstrators will be trained on one solving method, and the edge demonstrators will be trained in a different solving method. 
