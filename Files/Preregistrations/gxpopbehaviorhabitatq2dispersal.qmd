...
az_genepopfile<-read.table("/Users/dieterlukas/ownCloud/Documents/Grackles/GusGenetics/2023/AZpopulations.snps.txt",sep="\t",colClasses="character")

ca_genepopfile<-read.table("/Users/dieterlukas/ownCloud/Documents/Grackles/GusGenetics/2023/WOpopulations.snps.txt",sep="\t",colClasses="character")
...
ca_ade_input<-df2genind(ca_genepopfile_ade, ncode=2,NA.char="00")
...
apply(ca_genepopfile_ade[,1:493], 1, function(x) length(which(x=="0000")))
apply(ca_genepopfile_ade, 2, function(x) length(which(x=="0000")))
...
az_ade_input<-df2genind(az_genepopfile_ade, ncode=2,NA.char="00")
...
apply(az_genepopfile_ade[,1:462], 1, function(x) length(which(x=="0000")))
apply(az_genepopfile_ade, 2, function(x) length(which(x=="0000")))
...
CA
We retained 493 SNPs. Data was missing for 3.3% of all alleles (individuals missing information for either one or both of their chromosomes for that particular position). None of the SNP showed a particular underrepresentation of information. This missingness was due to the incomplete genotype of one individual (C116RY), who had missing data at 459 of the 493 SNPs, whereas all other individuals had data missing at four or less SNPs. All SNPs had 2 alleles and the observed heterozygosity (individuals carrying one copy each of the two bases) was 0.29, identical to the heterozygosity expected in a population with the same allele frequencies and random mating. The probability of identity for siblings, the chance that two siblings will show the same genotypes given the allele frequencies across these 493 SNP loci and random mating among individuals, is less than 10 to the power of minus 64. 

missing data CA / AZ

We retained 462 SNPs. Data was missing for 3.0% of all alleles (individuals missing information for either one or both of their chromosomes for that particular position). None of the SNP showed a particular underrepresentation of information. There were three individuals whose genotypes were less complete (A072KB at 191, 41% of SNPs; A088YR missing data at 174, 38%, of SNPs; A-059NB at 148, 32% of SNPs), whereas all other individuals had data missing at less than 10% of SNPs. All SNPs had 2 alleles and the observed heterozygosity (individuals carrying one copy each of the two bases) was 0.29, similar to the heterozygosity expected in a population with the same allele frequencies and random mating. The probability of identity for siblings, the chance that two siblings will show the same genotypes given the allele frequencies across these 462 SNP loci and random mating among individuals, is less than 10 to the power of minus 60. 
...
For discussion: In species with known philopatry/dispersal patterns, what is the average relatedness, and how many kin dyads are there?
...
az_relatedness_matrix[1,1] <- "A001YP"
...
#turn pairwise_r$wang into a matrix
ca_all_relatedness<-select(ca_pairwise_r,ind1.id,ind2.id,wang)
ca_relatedness_matrix<-spread(ca_all_relatedness,"ind1.id","wang")
ca_relatedness_matrix <- cbind(ca_relatedness_matrix,AF_061PR="NA")
...
ca_relatedness_matrix[1,1] <- "C101AY"
...
# ca_relatedness_matrix <- cbind(ca_relatedness_matrix,AF_061PR="NA")
# ca_relatedness_matrix<-arrange(ca_relatedness_matrix,ind2.id)
# ca_relatedness_matrix <- InsertRow(data=ca_relatedness_matrix, NewRow=rep("NA",samplesize[3,3]+samplesize[4,3]), RowNum=1)
# ca_relatedness_matrix[1,1] <- "A001YP"
# rownames(ca_relatedness_matrix)<-ca_relatedness_matrix[,1]
