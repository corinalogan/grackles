...
**This is the third of of three post-study manuscripts from the preregistration that was pre-study peer reviewed and received an In Principle Recommendation on 26 Mar 2019 by:**
...
$A_{r,t+1}$  = ((1-$\phi$) x $A_{r,t}$ + $\phi$ x $\pi$) x $P_{t}$ + (1-$P_{t}$) x $A_{r,t}$.

Equation 3b (dynamic association for the non-rewarded option): 

$A_{n,t+1}$ = (1-$P_{t}$) x (1-$\phi$) x $A_{n,t}$ + $P_{t}$ + (1-$P_{t}$) x $A_{n,t}$.
...
## 4) Comparing $\phi$ and $\lambda$ from the beginning and end of the observed serial reversal learning experiment to assess which changes more as grackles improve their performance
...
## 5) Calculating whether individual differences in $\phi$ and $\lambda$ persist throughout the serial reversal learning experiment and whether grackles differ in how much they change throughout the experiment

We checked whether the $\phi$ and $\lambda$ values of grackles at the beginning were associated with how much they changed (difference in values between beginning and end):
...
where $n_{b}$ is the average latency, counted as the number of seconds, to attempt a new option on the multi-option puzzle box, $m_{b}$ reflects the tendency of each grackle to wait (if they have a higher tendency to wait, they have a longer latency), $s$ controls the variance (larger values mean the overall distribution is more like a pure Poisson process in which all grackles have the same tendency to wait), $\alpha$ is the intercept, $\beta_{1}$ is the expected linear amount of change in latency for every one unit change in $\phi$, $\beta_{2}$ is the expected non-linear amount of change in latency for every one unit change in $\phi^{2}$, $\beta_{3}$ the expected linear amount of change for changes in $\lambda$, and $\beta_{4}$ is the expected non-linear amount of change for changes in $\lambda^{2}$.
