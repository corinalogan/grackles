...
2) Fat score data were distributed such that the majority of scores were 0, with some 1's and very few higher numbers. This zero inflation led to problems when we ran the models: it was difficult to fit models using an ordinal regression. The function "simulateResiduals", which we used to check our data, does not work with data in the ordinal family. Consequently, we modified the model to use a logistic regression where the dependent variable FatScore represents no fat (score = 0), or some fat (score = 1).
...
Neither SMI nor fat score differed by season in males or females (Fig. 3), therefore we did not include season as an independent variable in the final models.
