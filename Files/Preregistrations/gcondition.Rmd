...
```{r p1 results, eval=TRUE, warning=FALSE, results='asis', echo=TRUE, include=TRUE}
d2_f <- read.csv(url("https://raw.githubusercontent.com/corinalogan/grackles/master/Files/Preregistrations/condition_p1model_fdata.csv"), header=T, sep=",", stringsAsFactors=F)
d2_m <- read.csv(url("https://raw.githubusercontent.com/corinalogan/grackles/master/Files/Preregistrations/condition_p1model_mdata.csv"), header=T, sep=",", stringsAsFactors=F)
...
p1_f = data.frame("Parameter" = c("Intercept","SMI","Season"),"Females" = c("-0.20 (0.74)","0.07 (0.30)","0.27 (0.71)"), 
                "p-value" = c(0.79,0.81,0.70))
...
kables(list(kable(p1_f,align=c("l","l", "r"),valign="t",booktabs = TRUE),kable(p1_m,align=c("l","r"),valign="t",booktabs = TRUE,position ="float_right")),caption="Table 1: Results from the logistic mixed-effect regression for females and fixed-effect regression for males to determine whether fat score and scaled mass index (SMI) are correlated. Estimates are presented with the standard error in parentheses")
 ...
 kbl() %>%
  kable_styling() %>%
   add_header_above(c(" " = 1, "Females" = 2, "Males" = 2)) #%>%
    #col.names = c("Parameter", "Estimate (SE)","p-value","Estimate (SE)","p-value")
