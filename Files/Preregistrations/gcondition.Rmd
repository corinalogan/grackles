...
## UNREGISTERED ANALYSIS - had to tighten priors because random effect in H1 analysis was not converging
prior = list(R=list(R1=list(V=1,nu=0.002),R2=list(V=1,nu=0.002),R3=list(V=1,nu=0.002)), G=list(G1=list(V=1,nu=0.002)))
...
#Males
simulationOutput_m <- simulateResiduals(fittedModel = glm(as.factor(Fatbin) ~ Body2 + Season, family=binomial(link = "logit"), data=d2_m), n=250) #fit is singular with random effect included
...
rpt(log(wing) ~ Sex + (1|ID), grname = "ID", data = d3, datatype = "Gaussian", nboot = 500, npermut = 500) # WARNING - fit is singular - try testing sexes separately?
# R = 0.68, CI: 0.45 - 0.82, p = 0.002
rpt(log(wing) ~ (1|ID), grname = "ID", data = d[which(d$Sex == "F"),], datatype = "Gaussian", nboot = 500, npermut = 500)
rpt(log(wing) ~ (1|ID), grname = "ID", data = d[which(d$Sex == "M"),], datatype = "Gaussian", nboot = 500, npermut = 500)
# No, fit still singular for both
?isSingular


## UNREGISTERED ANALYSIS - Because fit was singular in the repeatability analysis (indicating model is overfit and parameters and error might be poorly estimated). 
r_wing1 = glm(log(wing) ~ Sex, data = d3)
r_wing2 = glmer(log(wing) ~ Sex + (1|ID), data = d3)
anova(r_wing2,r_wing1) 
# random effect significantly adds to model X2 = 22.4, p << 0.01; indicates measures are clustered within individual and therefore repeatable.



#GLMM 

## UNREGISTERED ANALYSES - Fat score data were distributed such that the majority of scores were 0, with some 1's and very few higher numbers. This made it difficult to fit models using ordinal regression.  Consequently, we used logistic regression where the dependent variable Fatbin represents no fat (score = 0), or some fat (score = 1+).
## UNREGISTERED ANALYSIS - models don't fit because Sex variable explains all of the variance Body2 and Sex effectively explain the same thing, multicollinearity.  Need to separate models for each sex.
m_condition <- MCMCglmm(Fatbin ~ Body2 + as.factor(Sex) + as.factor(Season), family="categorical", data=d2, verbose=F, prior=prior, nitt=200000, thin=50, burnin=100000)
summary(m_condition)
## No relationship between Fatbin and SMI p = 0.92; No relationship between Fatbin and Season p = 0.44 or Sex p = 0.69
autocorr(m_condition$Sol) #Did fixed effects converge? Values after Lag 0 should be < 0.01. 
# Fixed effects basically converge
#autocorr(m_condition$VCV) # Random effect looks bad when plot model


## UNREGISTERED ANALYSES
m_condition1 = glm(Body2 ~ FatScore + Sex + Season, family=gaussian, data=d2)
m_condition2 = glmer(Body2 ~ FatScore + Sex + Season + (1|Experimenter), family=gaussian, data=d2) #fit is singular
