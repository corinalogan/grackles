...
Measurements of energetic condition are important for understanding variation in life history characteristics in studies across the animal kingdom. However, the results of this study highlight the need to better understand proxy measures of energetic condition, not only in grackles, but for birds in general. Most studies on avian energetic condition only use one proxy variable, but because energetic condition is difficult to measure directly, it is important to compare multiple proxy variables to ensure each proxy is measuring the intended trait [the jingle-jangle fallacy; @block1995contrarian; @carter2013animal]. Future research could add to this work by incorporating additional methods that have been proposed to measure energetic condition, for example, blood hematocrit levels [@dawson1997avian] and protein storage [@houston1995changes], or by studying additional traits that could relate to variation in energy stores, such as dispersal [@ellers1998field], migratory endurance [@deppe2015fat] or survival [@liao2011fat]. Additionally, since SMI can perform poorly in birds with low lipid mass, future research should compare several mass by structural body size equations to determine the most appropriate proxy for a specific study system [@jacobs2012determining]. Furthermore, future research would benefit from using logistic exposure models to examine the relationship between energetic condition and reproductive success because these models control for the bias that arises when early nest failures are not detected, which is not possible in logistic regression models, and it is more sensitive to changes in a bird's nest status [@shaffer2004unified]. 
