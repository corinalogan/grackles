...
![**Figure 1.** Change in genetic relatedness as geographic distance among dyads increases. Each dot reflects a single dyad, a pair of female individuals (yellow) or a pair of male individuals (blue). There are very few close male relatives who are found at larger distances. The small number of close female relatives are all found within relatively short distances of each other. The dotted horizontal line indicates the level of relatedness for half-siblings (r=0.25).](gdispersal_Fig2_RelatednessDistance.png)
...
DNA from 57 great-tailed grackles was obtained from wild individuals caught in Tempe, Arizona, USA (see Figure 2 for a map). These individuals were either immediately released, or temporarily brought into aviaries for behavioral testing and then released back to the wild.

![**Figure 2.** Map displaying the sampling locations of grackles on the Arizona State University campus and the number of great-tailed grackles trapped at each location as part of this research.](gdispersalFig1_samplesites.png)
...
```{r dist_check4, eval=FALSE, warning=FALSE, results='asis', echo=TRUE}
...
```{r dist_check5, eval=FALSE, warning=FALSE, results='asis', echo=TRUE}
...
```{r dist_check6, eval=FALSE, warning=FALSE, results='asis', echo=TRUE}
...
```{r dist_check7, eval=FALSE, warning=FALSE, results='asis', echo=TRUE}
