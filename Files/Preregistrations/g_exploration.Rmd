...
To determine whether the experimenter (Breen) coded the dependent variables in a repeatable way, a hypothesis-blind video coder, Vincent Kiepsch, was first trained in video coding the dependent variable (order of the latency-distance categories), and then he coded 26% of the videos in the exploration and boldness experiments. We randomly chose five (Tomatillo, Queso, Mole, Habanero, and Tapa) of the 19 birds who participated in these experiments using random.org. Kiepsch then analyzed all videos from these five birds. The experimenter's data was compared with Kiepsch's data using the intra-class correlation coefficient (ICC) to determine the degree of bias in the regression slope (@hutcheon2010random, using the irr package in R: @gamer2012package). 

To pass **interobserver reliability (IOR) training**, Kiepsch needed an ICC score of 0.90 or greater to ensure the instructions were clear and that there was a high degree of agreement across coders. For training for the Exploration experiment, Kiepsch coded three videos (see R code comments for details). When Kiepsch's results were compared with the experimenter's, the score for the order of the latency-distance categories in Exploration was ICC=0.96 (confidence boundaries=0.92-1.00, n=141 data points), indicating that this variable was repeatable and he passed training. The boldness training score required additional training than the originally planned two videos, resulting in a final ICC=1.00 (confidence boundaries=1.00-1.00, n=11 data points; see R code comments for details). Note that, for both experiments, the ordered categories were aligned based on similar latencies between coders to prevent disagreements near the top of the data sheet from misaligning all subsequent entries.
 
**Interobserver reliability scores (26% of the videos)** were as follows:

   - Exploration and boldness: closest distance to apparatus ICC = 0.
   - Exploration environment: total number of times entered tent ICC = 0.
...
de <- d[d$Experiment=="Exploration2",] 
head(de)  #Check to make sure it looks right
...
# Exploration ENVIRONMENT: Total number of times entered tent



##### BOLDNESS

#### TRAINING: Did Vincent pass interobserver reliability TRAINING? NO. 2 Nov 2020: Retrain and redo training on new videos 
### ALEXIS: will you please list all of the boldness videos Vincent was trained on and which we ended up using only for training and which we ran the ICC on? I think the ICC was run only on the 2 files below (which are in the data sheet)
...
library(DHARMa)
library(lme4)
simulationOutput <- simulateResiduals(fittedModel = glmer(LatencyFirstLand ~ Condition + (1|BirdID), family=gaussian, data=ee), n=250) #250 simulations, but if want higher precision change n>1000
...
#p=0.056 so the data are not heteroscedastic
plot(simulationOutput) #...there should be no pattern in the data points in the right panel
...
#REPEATABILITY
#GLMM - 8 Feb 2021: changed family from poisson to gaussian because latency is gaussian
library(MCMCglmm)
prior = list(R=list(R1=list(V=1,nu=0)), G=list(G1=list(V=1,nu=0)))
expl <- MCMCglmm(LatencyFirstLand ~ Condition, random=~BirdID, family="gaussian", data=ee, verbose=F, prior=prior, nitt=13000, thin=10, burnin=3000)
summary(expl)
#autocorr(expl$Sol) #Did fixed effects converge? Most values=Yes
...
mean(repeata) #0.46
var(repeata) #0.01
posterior.mode(repeata) #0.51
HPDinterval(repeata, 0.95) #0.24-0.71
...
posterior.mode(repeataF) #0.42
HPDinterval(repeataF, 0.95) #0.27-0.71

# Now compare with the raw repeatability: null model
explraw <- MCMCglmm(LatencyFirstLand ~ 1, random=~BirdID, family="gaussian", data=ee, verbose=F, prior=prior, nitt=13000, thin=10, burnin=3000)
summary(explraw)
...
posterior.mode(repeata) #0.51
HPDinterval(repeata, 0.95) #0.23-0.71
```
...
#REPEATABILITY
#GLMM
eo2 <- subset(eo,!(is.na(eo["Condition"])))
library(MCMCglmm)
prior = list(R=list(R1=list(V=1,nu=0)), G=list(G1=list(V=1,nu=0)))
explo <- MCMCglmm(LatencyFirstLand ~ Condition, random=~BirdID, family="gaussian", data=eo2, verbose=F, prior=prior, nitt=63000, thin=100, burnin=23000)
summary(explo) #slower to approach the novel object compared with the familiar object
autocorr(explo$Sol) #Did fixed effects converge? No
autocorr(explo$VCV) #Did random effects converge? No
...
mean(repeata) #0.01
var(repeata) #0.001
posterior.mode(repeata) #-0.125e-07
HPDinterval(repeata, 0.95) #8.08e-23 to 0.08
...
posterior.mode(repeataF) #0.23
HPDinterval(repeataF, 0.95) #0.13-0.44

# Now compare with the raw repeatability: null model
exploraw <- MCMCglmm(LatencyFirstLand ~ 1, random=~BirdID, family="gaussian", data=eo2, verbose=F, prior=prior, nitt=13000, thin=10, burnin=3000)
summary(explraw)
...
posterior.mode(repeata) #-1.25e-07
HPDinterval(repeata, 0.95) #8.08e-13 to 0.07
```
...
library(lme4)
eo <- glmer(LatencyFirstLand ~ Test + (1|BirdID), family=gaussian, data=dexp)
#summary(eo) #negative correlation between environment and object latencies,r=-0.52
