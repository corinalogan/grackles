...
To determine whether experimenters coded the dependent variables in a repeatable way, a hypothesis-blind video coder, Vincent Kiepsch, was first trained in video coding the dependent variable (order of the latency-distance categories), and then he coded 26% of the videos in the exploration and boldness experiments. We randomly chose five (Tomatillo, Queso, Mole, Habanero, and Tapa) of the 19 birds who participated in these experiments using random.org. Kiepsch then analyzed all videos from these five birds. The experimenter's data was compared with Kiepsch's data using the intra-class correlation coefficient (ICC) to determine the degree of bias in the regression slope (@hutcheon2010random, using the irr package in R: @gamer2012package). 

To pass **interobserver reliability (IOR) training**, Kiepsch needed an ICC score of 0.90 or greater to ensure the instructions were clear and that there was a high degree of agreement across coders. For training, Kiepsch coded three videos: 1) A035P- 2018-12-28 ExploreObj Time1 Familiar T1, 2) A035P- 2018-12-27 ExploreEnv Time1 Novel T1, and 3) A035P- 2019-2-12 ExploreEnv Time2 Novel T1. When Kiepsch's results were compared with the experimenter's (Breen), the score for the order of the latency-distance categories was ICC=0.96, indicating that this variable is repeatable and he passed training.
...
**Interobserver reliability scores (26% of the videos)** were as follows:
   ...
   - Persistence: total number of touches to apparatus ICC = 0.
...
#### 1. Did Vincent pass interobserver reliability TRAINING? YES (5 Nov 2020)
...
d <- read.csv("/Users/corina/ownCloud/Documents/Grackles/Experiments/Interobserver Reliability/IOR_vincent.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE)
...
#LatencyFirstTimeWithin20cmOfObject
...
#### 1. Did Vincent pass interobserver reliability TRAINING? NO. 2 Nov 2020: Retrain and redo training on new videos
...
#### On 20% of the videos, what was the interrater reliability between Vincent and Alexis?
#LatencyFirstTouchToFood

#need to change the file path to the correect tab at the google sheet - the one here is for training data
d <- read.csv("/Users/corina/ownCloud/Documents/Grackles/Experiments/Interobserver Reliability/IOR_vincent.csv", header = TRUE, sep = ",", stringsAsFactors = FALSE)
head(d)  #Check to make sure it looks right

db <- d[d$Experiment=="Exploration2",] 
head(db)  #Check to make sure it looks right
