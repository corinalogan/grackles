...
> MOVE TABLE 3 to "Innovation analysis" section and delete the trials to reverse data from this table
...
>REPLACE FLEXIBILITY ANALYSIS MODEL AND SIMULATIONS WITH BELOW

We modified the reversal learning Bayesian model in @blaisdell2021causal to simulate and analyze population differences in reversal learning, and calculate our ability to detect differences between populations. As in @blaisdell2021causal, we, too, used previously published data on reversal learning of color tube preferences in great-tailed grackles [@logan2016behavioral] to inform the model. 


```{r sim_power_philambda, eval=F, warning=FALSE, results='asis', echo=TRUE, include=TRUE}
library(rethinking)
library(dplyr)
library(rethinking)

# We first reanalyze the Santa Barbara data to otbtain values for phi and lambda upon which to base our expectations of what the mean and variance might be at a site.
...
simulatedreversaldata_attractionscores_1<-read.csv(url("https://raw.githubusercontent.com/corinalogan/grackles/master/Files/Preregistrations/gxpopbehaviorhabitat_SimulatedReversalData_Grackles_PhiLambda_Attraction04_Aug2021.csv
gxpopbehaviorhabitat_SimulatedReversalData_Grackles_PhiLambda_Attraction02_Aug2021.csv"), header=T, sep=",", stringsAsFactors=F) 
...
### SIMULATION: Manipulate mu sd of phi across a set of 7 populations
...
currentmeandifference<-rep(c(0.01, 0.024, 0.025, 0.028, 0.04, 0.05, 0, 0.01,  0.024, 0.025, 0.028, 0.04, 0.05, 0.014, 0.015, 0.018, 0.03, 0.04, 0.01, 0, 0.014, 0.015, 0.018, 0.03, 0.04, 0.001, 0.004, 0.016, 0.026, 0.024, 0.014, 0, 0.001, 0.004, 0.016, 0.026, 0.003, 0.015, 0.025, 0.025, 0.015, 0.001, 0, 0.003, 0.015, 0.025, 0.012, 0.022, 0.028, 0.018, 0.004, 0.003, 0, 0.012, 0.022, 0.01, 0.04, 0.03, 0.016, 0.015, 0.012, 0, 0.01, 0.05, 0.04, 0.026, 0.025, 0.022, 0.01, 0, 0.01, 0.024, 0.025, 0.028, 0.04, 0.05, 0.014, 0.015, 0.018, 0.03, 0.04, 0.001, 0.004, 0.016, 0.026, 0.003, 0.015, 0.025, 0.012, 0.022, 0.01) ,60)
currentoutcome<-NA
counter<-1
...
for (samplesize in 1:60) {
        ...
        # Now, we extract the posterior of the model, and on this perform the comparison to see whether the model thinks the sites have different or the same phi. For this, we calculate all pairwise differences in the posterior estimates of the sites.
        for (sample in 1:13) {
        ...
        currentsamplesize[counter]<-samplesizes[i]
        counter<-counter+1
           }
        }
}
...
#To get a better estimate of how the probability that the model will estimate that there is a difference, we use these outputs in another STAN model. This model will, for each pairwise difference, calculate how the sample size influences whether a comparison was deemed different or not

# We set up the data frame for the model
falsenegative_data <- list(differencedetected = falsenegative$differencedetected,
                           populationdifference = 1/(1100*(falsenegative$populationdifference)),
...
precis_output_5<-precis( output_5 , depth=2 )

# Based on the posterior, we want to estimate the predicted performance at the sample size. We set up vectors to store these perdictions
samplesize_9_predictions<-NA
samplesize_12_predictions<-NA
samplesize_15_predictions<-NA
samplesize_18_predictions<-NA
samplesize_21_predictions<-NA
samplesize_24_predictions<-NA


# There are 17 unique differences in the mean phis of the populations (e.g. the difference between a mean of 0.01 and 0.02 is the same as the difference between a mean of 0.025 and 0.035). For each of these, we generate the predictions for each sample size
for (i in 1:17) {
  samplesize_9_predictions[i]<-inv_logit(precis_output_5[1,1])-inv_logit(precis_output_5[9,1])*sequence_mean_differences[i]
}
for (i in 1:17) {
  samplesize_12_predictions[i]<-inv_logit(precis_output_5[2,1])-inv_logit(precis_output_5[10,1])*sequence_mean_differences[i]
}
for (i in 1:17) {
  samplesize_15_predictions[i]<-inv_logit(precis_output_5[3,1])-inv_logit(precis_output_5[11,1])*sequence_mean_differences[i]
}
for (i in 1:17) {
  samplesize_18_predictions[i]<-inv_logit(precis_output_5[4,1])-inv_logit(precis_output_5[12,1])*sequence_mean_differences[i]
}
for (i in 1:17) {
  samplesize_21_predictions[i]<-inv_logit(precis_output_5[5,1])-inv_logit(precis_output_5[13,1])*sequence_mean_differences[i]
}
for (i in 1:17) {
  samplesize_24_predictions[i]<-inv_logit(precis_output_5[6,1])-inv_logit(precis_output_5[14,1])*sequence_mean_differences[i]
}


# We can now plot the predictions for the different sample sizes as separate lines
sequence_mean_differences_toplot<-sort(unique(falsenegative$populationdifference),decreasing=T)
sequence_mean_differences_toplot[17]<-0.001
samplesize_9_predictions[17]<-0
plot(lines(samplesize_9_predictions~sequence_mean_differences_toplot,col="black"),xlim=c(0,0.05),ylim=c(0.0,1.0),pch=1,ann=F,frame.plot=F,cex=1.5)
lines(1-samplesize_9_predictions~sequence_mean_differences_toplot,col="grey75",lty=2,lwd=3)
lines(1-samplesize_12_predictions~sequence_mean_differences_toplot,col="red",lty=1,lwd=5)
lines(1-samplesize_15_predictions~sequence_mean_differences_toplot,col="blue",lwd=1.5,lty=3)
lines(1-samplesize_18_predictions~sequence_mean_differences_toplot,pch=1,col="yellow",lty=1,lwd=3)
lines(1-samplesize_21_predictions~sequence_mean_differences_toplot,pch=1,col="grey75",lty=2,lwd=3)
lines(1-samplesize_24_predictions~sequence_mean_differences_toplot,pch=1,col="black",lty=3,lwd=3)

legend(x="topright",legend=c(lty2="9",lty1="12",lty3="15",lty1="18",lty2="21",lty3="24"),lty=c(2,1,3,1,2,3),lwd=c(3,3,3,3,3,3),col=c("grey75","red","blue","yellow","grey75","black"),cex=1,bty="n")
...
<img width="100%" src="gxpopbehaviorhabitat_Figure_Power_PhiLambda.png">

**Figure X.** How small of a site difference in phi and lambda can we detect? The probability that the model estimates that the difference shown on the x axis is zero, meaning that these two estimates come from a population with the same phi or lambda. Each point is the mean phi or mean lambda from one site minus the mean phi or mean lambda from another site (calculated from 20 individuals per site) for all pairwise comparisons for all 32 sites (for a total of 496 pairwise comparisons). Left panels: error bars=89% compatability intervals. Right panels: shaded areas=97% prediction intervals.
