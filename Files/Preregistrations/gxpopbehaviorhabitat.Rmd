...
As in @blaisdell2021causal, we, too, used previously published data on reversal learning of color tube preferences in great-tailed grackles [@logan2016behavioral] to inform the model modifications. We modified the @blaisdell2021causal model in a few ways: 1) we set the attraction score to option 1 and option 2 (the light gray and dark gray tubes) to 0.1 rather than 0.0. This assumed that there would be some inclination (rather than no inclination) for the bird to approach the tube when it was available because it had been previously trained to do so. This also allowed the attraction score to decrease when a non-rewarded choice is made near the beginning of the experiment. 2) We gave each of the two choice options (light gray and dark gray tubes) their own attraction score, which means that they only learn about the color they choose and only this color’s attraction score subsequently gets updated. This is instead of there only being one attraction score that encompasses both choices, which assumes that if a bird chooses one color, they learn that that color did or did not have the reward, and at the same time it is inferred that they were able to learn something about the color they did not choose. Our modification resulted in allowing a higher $\phi$ to have the same learning rate as a model where only one attraction score accounts for both choices. In a single step if you update both at the same time you learn twice as fast as when you only update one at a time. This allowed the $\lambda$ to be low enough for them to be able to continue exploring during the initial reversal trials.

We first reanalyzed the Santa Barbara grackle data to obtain the phi and lambda values, which informed our expectations of what a site’s mean and variance might be. Then we ran simulations, where we determined that we wanted to make the previously mentioned modifications to the stan [@stan2016stan] model [in R, current version `r getRversion()`; @rcoreteam]. This model will be used to analyze the actual data after it is collected. 

**Power analyses:** We also used the simulations to estimate our ability to detect differences in $\phi$ and $\lambda$ between sites based on extracting samples from the posterior distribution. We found that we are able to detect that two sites are different from each other if the difference in their $\phi$ is 0.1, assuming that 20 individuals are sampled per site. Figure Y shows how different site sample sizes change detection levels: once a sample size of 15 is reached, there are only minimal differences in detection abilities compared to larger sample sizes. We are able to detect that two sites are different from each other if the difference in their $\lambda$ is 1, assuming 20 individuals are sampled per site. Figure Y shows that once a sample size of 15 is reached, there are only minimal differences in detection abilities compared to larger sample sizes.
