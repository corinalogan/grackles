...
output:
  ...
  github_document: 
    toc: true
bibliography: https://raw.githubusercontent.com/corinalogan/grackles/master/Files/MyLibrary.bib
...
![**Figure 1.** What plays a larger role in a rapid range expansion: behavior changes or suitable habitat changes? A visual representation of Q1 (top) and Q2 (bottom).](gxpopbehaviorhabitatFig1.png)
...
#### Q3 (suitable habitat GTGR & BTGR): Are there differences in the availability of habitat between great-tailed grackles (GTGR) and boat-tailed grackles (BTGR)? (Fig. 3; @wehtje2003range, @selander1961analysis)

**Prediction 3:** **GTGR and BTGR use different habitats, and the habitat of GTGR, but not that of BTGR, has increased in suitability and connectivity over the past few decades**. *This supports the hypothesis* that the availability of habitat, not inherent species differences, explains why great-tailed grackles (GTGR) are able to much more rapidly expand their range than boat-tailed grackles (BTGR).
...
![**Figure 3.** Ecological niche modeling of the presence/absence of great-tailed grackles (GTGR; left), which are rapidly expanding their geographic range, and boat-tailed grackles (BTGR; right), which are not. The presence/absence map layer will be overlaid over habitat layers to determine habitat suitability and availability. Map credit: eBird.org.](gxpopbehaviorhabitatFig3.png)
...
#### Protocols and open materials
  ...
  - Experimental protocols for Q1 are online [here](https://docs.google.com/document/d/1oqucJ4mH-glh9Hv9JaiDSyhEGi0yVpMuqqzqR2IwQeA/edit?usp=sharing).
...
The data will be analyzed as in @sevchik2019dispersal. To summarize, blood is collected from the bird, DNA is extracted (by Aaron Blackwell at Washington State University), size selected (between 400-700 base pairs), and sequenced using ddRADseq (at Cornell University Lab of Ornithology) on an Illumina NextSeq500 machine using the mid-output setting for 150 base pair single end reads. Data are post processed to generate single nucleotide polymorphisms (SNPs) as in @thrasher2018double. Genetic relatedness between all pairs of individuals is calculated using the package “related” (@pew2015related) in R (as in @thrasher2018double) using the estimator by Queller & Goodnight, which appeared most robust for the inferences in a subset of the data [@sevchik2019dispersal]. Permutations (i.e., randomly assigning site ID to individuals) and general linear models estimating average relatedness of each individual to all others at that site will be used to determine whether individuals at one site are more closely related to each other than the individuals at another site.
...
1) **Land cover** (e.g., forest, urban, crop land, coastal marsh, coastal prairie, coastal plain, grassland, brush, mangrove, distance from road/water body/wetland/water treatment plant) - we chose these land cover types because they represent the habitat types in which both species exist, as well as habitat types (e.g., forest) they are not expected to exist in [@selander1961analysis] to confirm that this is the case. If it is the case, it is possible that large forested areas are barriers for the range expansion of one or both species.

2) **Elevation** - @selander1961analysis notes the elevation range for GTGR (0-2134m), but not BTGR, therefore establishing the current elevation ranges for both species will allow us to determine whether and which mountain ranges present range expansion challenges.

3) **Climate** (e.g., daily/annual temperature range) - because this species was originally tropical [@wehtje2003range], which generally has a narrow daily and annual climate range, and now they exist in temperate regions, which have much larger climate ranges, this variable will allow us to determine potential climatic limits for both species. If there are limits, this could inform the difference between the range expansion rates of the two species.

4) **Predator density** - the reported predators of adult GTGR are Aplomado falcons (*Falco femoralis*), red-tailed hawk (*Buteo jamaicensis*), burrowing owl (*Athene cunicularia*), cottonmouth (*Agkistrodon piscivorus*), fox squirrels (*Sciurus niger*), and domestic cats (*Felis catus*), while domestic dogs (*Canis familiarus*) additionally prey on fledglings [@johnson2001great]. While there are no reported predators of adult BTGR, nests and young are predated by yellow snakes (*Elaphe obsoleta*), Black rats (*Rattus rattus*), Norway rats (*Rattus norvegicus*), rice rats (*Oryzomys palustris*), alligators (*Alligator mississippiensis*), and purple gallinules (*Porphyrio palustris*) [@post2020boat]. An increased density of predators in an area could be a barrier to the range expansion of one or both species. Predator density will be measured as the presence or absence of each predator species in a given habitat patch.

5) **Distance to the next suitable habitat patch weighted by nearest mountain range/forest** - given that GTGR are not found in forests and that the elevation limits for GTGR [@selander1961analysis], and observing the sightings of both species on eBird.org, large forests, tall mountain ranges and high elevation geographic features could block or slow the expansion of one or both species into these areas and their surroundings. We explore this possibility by weighting the distance to the next suitable habitat patch by the nearest forest and mountain range.

6) **Distance to the nearest conspecific population 10 years previous to the point in time being investigated** - we want to be able to predict presence or absence of each species in a given patch of habitat. The variables above represent habitat suitability, which could occur anywhere in the world. However, for the question of whether or not one of these species is in a particular patch, it also matters whether it would be possible for these New World-limited species to access that patch. The question is not whether there is currently an inhabited habitat patch nearby, it is whether in a time frame that would permit colonization there was an inhabited patch within migration distance. The migration distances are unknown in these species, therefore we will estimate these from the rate of spread that has been observed every 10 years for each species. We define conspecific population as this species being observed at a given location in at least 10 out of 12 months of the calendar year, and with at least 5 individuals observed at one time in at least one sighting.
...
**Analysis 1:** are there differences in habitat characteristics within and between species over time?
  - Variable 1: the habitat characteristics that determine the current (~2018) range of both species
  - Variable 2: the habitat characteristics that determined the range of both species in 1970

**Analysis 2:** has the available habitat for both species increased over time? Using the habitat variables, determine the amount of area where grackles from both species could potentially live.
  - Variable 1: the habitat characteristics that determine the current (~2018) range of both species
  - Variable 2: the habitat characteristics that determined the range of both species in 1970
