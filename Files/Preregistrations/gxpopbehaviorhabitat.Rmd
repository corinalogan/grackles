...
Equation 2 “expresses the probability an individual $j$ chooses option $i$ in the next round, $t+1$, based on the latent attractions. The parameter $\lambda_j$ represents the *random choice rate* of an individual (also called inverse temperature). It controls how sensitive choices are to differences in attraction scores. As $\lambda_j$ gets larger, choices become more deterministic, as it gets smaller, choices become more exploratory (random choice if $\lambda_j = 0$). For instance, if an individual has a 0.6-0.4 preference for option A, a value of $\lambda_j = 3$ means they choose A 65% of the time, a value of $\lambda_j = 10$ means they choose A 88% of the time and a value of $\lambda_j = 0.5$ means they choose A only 53% of the time” [@blaisdell2021causal].  

As in @blaisdell2021causal, we, too, used previously published data on reversal learning of color tube preferences in great-tailed grackles [@logan2016behavioral] to inform the model modifications. We modified the @blaisdell2021causal model in a few ways: 1) we set the initial attraction score to option 1 and option 2 (the light gray and dark gray tubes) to 0.1 rather than 0.0. This change assumes that there would be some inclination (rather than no inclination) for the bird to approach the tubes when they are first presented because it had been previously trained to expect food in tubes. This also allowed the attraction score to decrease when a non-rewarded choice is made near the beginning of the experiment. With the previous initial attraction scores set to zero, a bird would be expected to choose the rewarded option 100% of the cases after the first time it chose that option (attraction cannot be lower than zero, and choice is shaped by the ratio of the two attractions so that when one option is zero and the other larger than zero, the ratio will be 100% for the rewarded option).  2) We changed the updating so that a bird would only change the attraction toward the option they had chosen during that trial (either decreasing their attraction toward the unrewarded option or increasing their attraction toward the rewarded option). In the previous setting, both attractions were updated after every trial, assuming that individuals understand the setup of the experiment is that always one side is rewarded. For our birds, we rather assumed that individuals will focus on their direct experience rather than making abstract assumptions about the test. Our modification resulted in needing a higher $\phi$ to have the same learning rate as a model where both attraction scores are updated after every trial. This change also appeared to better reflect the performance of the Santa Barbara birds, because birds had higher $\phi$ values, which in turn meant lower $\lambda$ values to reflect the performance during the intial learning. These lower $\lambda$ values better reflect the birds' behavior during the first reversal trials: a large $\lambda$ value would mean that birds continue to choose the now unrewarded option almost 100% of the time, whereas the lower $\lambda$ values reflect that birds start to explore the rewarded option relatively soon after the switch of the rewarded option.

We first reanalyzed the Santa Barbara grackle data to obtain the phi and lambda values with this revised model, which informed our expectations of what a site’s mean and variance might be. Then we ran simulations, where we determined that we wanted to make the previously mentioned modifications to the stan [@stan2016stan] model [in R, current version `r getRversion()`; @rcoreteam]. This model will be used to analyze the actual data after it is collected. 

**Power analyses:** We also used the simulations to estimate our ability to detect differences in $\phi$ and $\lambda$ between sites based on extracting samples from the posterior distribution. We run two different sets of simulations: the first set of simulations recreated choices for 20 birds per population across initial learning and reversal trials; the second set of simulations sampled between 9 and 24 birds from populations with pre-specified phi and lambda means. The first set of simulations showed that we have a very high chance to detect that two sites are different from each other if the difference in their $\phi$ is 0.01 and/or if the difference in their $\lambda$ is 3, assuming that 20 individuals are sampled per site (Figure X, four panels, gxpopbehaviorhabitat_Figure_Power_PhiLambda). Figure Y (two panels, xpopbehavhabitat_Figure_PowerCombinedSampleSizes), based on the second set of simulations, shows how different site sample sizes change detection levels: once a sample size of 15 is reached, there are only minimal differences in detection abilities compared to larger sample sizes. It appears that there is more variability in the $\lambda$ estimates for each bird based on their choices, meaning that with this model the differences between sites have to be larger in order to be reliably detected.
