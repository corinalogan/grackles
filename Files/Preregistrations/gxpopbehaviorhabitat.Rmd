...
![**Figure 4.** Ecological niche modeling will allow us to answer our research questions about the role of habitat changes for the rapid range expansion of GTGR. We will assess similarities and differences between GTGR and BTGR regarding suitable habitat (P3), habitat connectivity (P3), habitat breadth (P4), and whether differences might be due to an inherent species trait (P5).](gxpopbehaviorhabitatFig4.png)
...
d <- read.csv(url("https://raw.githubusercontent.com/corinalogan/grackles/master/Files/Preregistrations/gxpopbehaviorhabitat_Table3.csv"), header=F, sep=",", stringsAsFactors=F) 
...
#### NOW RUN the simulation while changing the VARIANCE between sites (the standard deviation of the probability of solving a locus at each site) and holding sample size and means constant and equal
#RUN SIMULATION WITH PARAMETERS (this is the simulated data that the model (below) will use) Generate values for simulated individuals from 3 simulated sites: vary the alpha site mean (expected number of loci solved and whether sites differ) and alpha sd (the noise in the population in the number of loci solved), and sample size
asd <- 0.3 #alpha sd for the population. We set this smaller than what we had it above bc we wanted to make it so individuals were not equally likely to have similar results
s1mean <- 2 #site 1: alpha mean: solves on average 2 loci 
s2mean <- 2 #site 2
s3mean <- 2 #site 3
n <- 45
...
for (i in ((n/3)+1):((n/3)+(n/3))){
  s2 <- rnorm(1,s2mean,1) #n, mean, sd of probability of solving a locus. SD is the noise
  p <- s2
  sites[i,1]<-sum(rbinom(50,1,inv_logit(p)/12.5)) 
  ifelse(sites[i,1]>4,sites[i,1]<-4,sites[i,1]<-sites[i,1])
  sites[i,2]<-2 #column: site 2
...
for (i in ((n/3)+(n/3)+1):n){
  s3 <- rnorm(1,s3mean,2) #n, mean, sd of probability of solving a locus. SD is the noise
  p <- s3
  sites[i,1]<-sum(rbinom(50,1,inv_logit(p)/12.5)) 
  ifelse(sites[i,1]>4,sites[i,1]<-4,sites[i,1]<-sites[i,1])
  sites[i,2]<-3 #column: site 3
...
**Analysis 1 (P3: different habitats):** does the range of variables that characterize suitable habitat for GTGR differ from that of BTGR? The habitat variables for each species will be visualized in a figure that shows the ranges of each variable for each species and how much they overlap or not.

**Analysis 2 (P3: habitat suitability):** has the available habitat for both species increased over time? Take the distribution of both species in 1970 and determine for each variable, the range in which they exist, this is then habitat suitability (defined for each species). Then take these variable ranges and identify what locations in the Americas fall within these ranges in 1970 and in 2018. We will then be able to determine whether the amount of suitable habitat has increased. 

**Analysis 3 (P3: habitat connectivity):** has the habitat connectivity for both species increased over time? If the connectivity distances are smaller in 2018, this will indicate that habitat connectivity has increased over time. To compare the distances between 1970 and 2018, and between the two species, we will run two models where both have the distance as the response variable and a random effect of location to match the points over time. The explanatory variable for model 1 will be the year (1970, 2018), and for model 2 it will be the species (GTGR, BTGR).

**Analysis 4 (P4: habitat breadth):** has the habitat breadth of both species changed over time? 

**Analysis 5 (P5 unused habitat):** no separate analysis needs to happen to determine this because it is an outcome from Analysis 2.
