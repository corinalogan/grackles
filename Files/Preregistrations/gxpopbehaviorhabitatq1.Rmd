...
We use a normal distribution for $\alpha$ because it is a sum [see Figure 10.6 in @statrethinkingbook]. We set the mean to 0 (on a logit scale, which is a probability of 0.5 that a bird will participate in every other trial on average on the actual scale).
...
**Table 1.5.** Contrasts (indicated by "diff") between populations for the flexibility measure of reversal learning: phi and lambda.

```{r resltsflextable, eval=T, warning=FALSE, results='asis', include=TRUE}
library(kableExtra)
sumtable <- read.csv(url("https://raw.githubusercontent.com/corinalogan/grackles/master/Files/Preregistrations/gxpopbehaviorhabitatq1dataSummaryCAAZ.csv"), header=F, sep=",", stringsAsFactors=F)
sumtable <- data.frame(sumtable)
colnames(sumtable) <- c("Population","Batch","ID","Bird name","Sex","Learning speed","Reversal speed","Phi","Lambda","MAB loci solved","MAB max loci available","Exploration latency","Persistence","Flexibility Manipulated","Notes")
...
d <- select(d0,Site,id,Reversal,Trial,Correct,Choice,Treatment)
...
dat$N_treat <- length(unique(d$Treatment))
...
parameters{
  ...
  // Varying effects clustered on treatment
  matrix[2,N_treat] z_TREAT;
  vector<lower=0>[2] sigma_TREAT;
  cholesky_factor_corr[2] Rho_TREAT;
...
matrix[N_treat,2] v_TREAT; // varying effects on site

v_ID = ( diag_pre_multiply( sigma_ID , Rho_ID ) * z_ID )';
v_EXP = ( diag_pre_multiply( sigma_EXP , Rho_EXP ) * z_EXP )';
v_TREAT = ( diag_pre_multiply( sigma_TREAT , Rho_TREAT ) * z_TREAT )';
}
...
to_vector(z_TREAT) ~ normal(0,1);
sigma_TREAT ~ exponential(1);
Rho_TREAT ~ lkj_corr_cholesky(4);
...
// first, what is log-prob of observed choice
L =  exp(log_L + v_ID[id[i],1]+ v_EXP[Site[i],1] + v_TREAT[Treatment[i],1] );
p = softmax(L*A[id[i],1:2]' );
Choice[i] ~ categorical( p );

// second, update attractions conditional on observed choice
phi =  inv_logit(logit_phi + v_ID[id[i],2]+ v_EXP[Site[i],2] + v_TREAT[Treatment[i],2] );
pay[1:2] = rep_vector(0,2);
pay[ Choice[i] ] = Correct[i];
A[ id[i] , Choice[i] ] = ( (1-phi)*(A[ id[i] , Choice[i] ]) + phi*pay[Choice[i]]);
}//i
...
site1lambda <- exp( mean(drawsdf$log_L) + mean(drawsdf$v_EXP.1.1.)) #4.78. New: 3.80
site2lambda <- exp( mean(drawsdf$log_L) + mean(drawsdf$v_EXP.2.1.)) #4.52. New: 4.41
site1phi <- inv_logit( mean(drawsdf$logit_phi) + mean(drawsdf$v_EXP.1.2.)) #0.0306. New: 0.008
site2phi <- inv_logit( mean(drawsdf$logit_phi) + mean(drawsdf$v_EXP.2.2.)) #0.0301. New: 0.01
...
#added treatment
site1lambda <- exp( mean(drawsdf$log_L) + mean(drawsdf$v_EXP.1.1. + mean(drawsdf$v_TREAT.1.1.)) #4.78. New: 3.80
site2lambda <- exp( mean(drawsdf$log_L) + mean(drawsdf$v_EXP.2.1.)) #4.52. New: 4.41
site1phi <- inv_logit( mean(drawsdf$logit_phi) + mean(drawsdf$v_EXP.1.2.)) #0.0306. New: 0.008
site2phi <- inv_logit( mean(drawsdf$logit_phi) + mean(drawsdf$v_EXP.2.2.)) #0.0301. New: 0.01
...
#NEW: site1lambdap...site2lambdap -0.6 1.73 -3.54  1.69 ▁▁▁▁▂▇▅▁▁▁▁▁▁

phip <- precis(site1phip - site2phip)
#                      mean   sd  5.5% 94.5% histogram
#NEW: site1phip...site2phip    0 0.01 -0.01  0.01 ▁▁▁▇▃▁▁▁▁
...
#1  Woodland          1 0.005025974  1.0127106        Chocolate
#2  Woodland          2 0.021621891 14.5147555             Flan
#3  Woodland          3 0.004740958  2.2717331        Tembleque
#4  Woodland          4 0.055570206  1.4246260       Polvorones
#5  Woodland          5 0.009930123  0.6579345          Alegria
#6  Woodland          6 0.014564132 11.1533515             Piña
#7  Woodland          7 0.009123372 17.2135277           Camote
#8  Woodland          8 0.007690806  1.3931624           Helado
#9  Woodland          9 0.006954332  1.9616663            Xango
#10 Woodland         10 0.021657240  8.5457965          Buñuelo
#11 Woodland         11 0.006694781 19.6841994   Dulce de Leche
#12 Woodland         12 0.005436245 23.0697127     Zapote Negro
#13 Woodland         13 0.004514605  1.0815169          Carlota
#14 Woodland         14 0.007356373  5.8102590           Ak'xi'
#15 Woodland         15 0.016371734  4.7047307              Kau
#16 Woodland         16 0.003228967  9.8302789              Kel
#17 Woodland         17 0.016428310  2.8589414         Galandra
#18 Woodland         18 0.003499246 28.8712050           Cuervo
#19 Woodland         19 0.006313674  1.5873905           Wachil
#20 Woodland         20 0.064771491  0.7684805            Xunub
#21 Woodland         21 0.017501973  5.1350576 Tzanatl preciosa
#22 Woodland         22 0.007420369 12.3006275         Cocinera
#23 Woodland         23 0.049399499  1.2704300            Cutuy
#24    Tempe         24 0.029351115  0.9858425        Tomatillo
#25    Tempe         25 0.011285440  2.0186226          Chalupa
#26    Tempe         26 0.001784257  4.6336053          Avocada
#27    Tempe         27 0.089872709  1.0376747           Diablo
#28    Tempe         28 0.017406539  2.6117934            Adobo
#29    Tempe         29 0.005165214 11.3492524            Queso
#30    Tempe         30 0.008475526  3.9762807         Habanero
#31    Tempe         31 0.007772918 10.5666930             Mole
#32    Tempe         32 0.007341076  4.9404011             Tapa
#33    Tempe         33 0.008944730  1.6154161           Memela
#34    Tempe         34 0.034646730  0.7770836            Fideo
#35    Tempe         35 0.008585882  9.0876639             Taco
#36    Tempe         36 0.011817485  7.8872108          Burrito
#37    Tempe         37 0.004244561 22.1329093          Marisco
#38    Tempe         38 0.011734945 32.6093184       Chilaquile
#39    Tempe         39 0.019836784  5.1663508             Yuca
#40    Tempe         40 0.002626128 18.7746893            Pizza
#41    Tempe         41 0.020792644  1.0400581          Mofongo
#42    Tempe         42 0.044698119  0.6393615          Pollito
#43    Tempe         43 0.011950185  2.7473288          Taquito
...
#NEW = same as before
#phi         0.02 0.02 0.00  0.05 ▂▇▁▂▁▁▁▁▁▁▁▁▁
#lambda      7.70 8.12 0.82 22.36        ▇▂▂▁▁▁
...
#NEW = same as before
#phi         0.02 0.02  0.00  0.04  ▇▅▂▁▁▁▁▁▁
#lambda      7.23 8.46  0.79 21.98    ▇▂▁▁▁▁▁

# visualize each bird at each site
library(lattice)
library(gridExtra)
plot1 <- bwplot(indphisite~individualsatsite$Site, d, cex=1.2, ylab="phi", col="black", panel = function(...) {
  panel.bwplot(..., pch = "|")
...
plot(datinn$proportion~datinn$plotlocation,xaxt="n",xlab="",ylab="Proportion of loci solved on the multiaccess box",cex.lab=1.5,cex=1.5,cex.axis=1.5)
...
library(kableExtra)
knitr::kable(d) %>%
kable_styling(full_width = T, position = "left", font_size = 8)
```
...
# exclude the birds who didn't complete reversal 1? no because not using this data to predict performance on the MAB or anything, just want to see if there are differences with GTGR
