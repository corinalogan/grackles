...
### Run the model on the actual data to determine what the probability is that this is a true result
...
Pr_positive_true <- 0.4 #1000 - num_crosses zero in persist_actualresults
Pr_positive_false <- 0.3 #1000 - num_crosses zero in persist_actualresults
Pr_positive <- Pr_positive_true / (Pr_positive_true + Pr_positive_false) #there is a % probability that this is a true result

### Writing the new code for rchunk resultspersist
participated ~ dbinom(totaltrials,p)
p~dnorm(a[site]+b*treatment,sigma[site])
a[site]~dnorm(abar,asd)
abar~dunif(0,1)
asd~dexp(1)
b~dunif(0,1)
sigma[site]~dexp(1)
