...
site1lambda <- exp( mean(drawsdf$log_L) + mean(drawsdf$v_EXP.1.1.)) #4.22
site2lambda <- exp( mean(drawsdf$log_L) + mean(drawsdf$v_EXP.2.1.)) #3.85
site1phi <- inv_logit( mean(drawsdf$logit_phi) + mean(drawsdf$v_EXP.1.2.)) #0.07787294
site2phi <- inv_logit( mean(drawsdf$logit_phi) + mean(drawsdf$v_EXP.2.2.)) #0.07544084
...
site1lambdap <- exp( (drawsdf$log_L) + (drawsdf$v_EXP.1.1.) + (drawsdf$v_TREAT.1.1.)) 
site2lambdap <- exp( (drawsdf$log_L) + (drawsdf$v_EXP.2.1.) + (drawsdf$v_TREAT.1.1.)) 
site1phip <- inv_logit( (drawsdf$logit_phi) + (drawsdf$v_EXP.1.2.) + (drawsdf$v_TREAT.1.2.)) 
site2phip <- inv_logit( (drawsdf$logit_phi) + (drawsdf$v_EXP.2.2.) + (drawsdf$v_TREAT.1.2.)) 
...
site2lambdap_manip <- exp( (drawsdf$log_L) + (drawsdf$v_EXP.2.1.) + (drawsdf$v_TREAT.2.1.)) 
site2phip_manip <- inv_logit( (drawsdf$logit_phi) + (drawsdf$v_EXP.2.2.) + (drawsdf$v_TREAT.2.2.)) 

# and run a contrast on this scale too to get the range
lambdap <- precis(site1lambdap - site2lambdap)
#                            mean   sd  5.5% 94.5%     histogram
#site1lambdap...site2lambdap 0.41 0.74 -0.67  1.67 ▁▁▁▁▁▅▇▅▂▁▁▁▁▁

phip <- precis(site1phip - site2phip)
#                     mean   sd  5.5% 94.5% histogram
#site1phip...site2phip    0 0.01 -0.01  0.01 ▁▁▁▁▁▁▂▇▇▃▂▁▁▁
# the site differences are unlikely to be larger than 0.1 for phi and 1.67 for lambda
...
for (i in 1:dat$N_id){
  indlambdasite[i] <- exp( mean(drawsdf$log_L) + mean(drawsdf[,i+148]) + 
  ...
  indphisite[i] <- inv_logit( mean(drawsdf$logit_phi) + mean(drawsdf[,i+105])+
...
library(lattice)
library(gridExtra)
plot1 <- xyplot(indphisite~individualsatsite$Site, d, cex=1.2, ylab=expression(Phi), col="black", pch=c(1,4)[d$Treatment]), panel = function(...) {
  #panel.xyplot(..., pch = "|")
  panel.xyplot(..., jitter.x = TRUE)})
plot2 <- xyplot(indlambdasite~individualsatsite$Site, d, ylab=expression(Lambda), col="black", panel = function(...) {
  #panel.xyplot(..., pch = "|")
  panel.xyplot(..., jitter.x = TRUE)})
