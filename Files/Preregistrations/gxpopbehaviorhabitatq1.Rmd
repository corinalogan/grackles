...
site1means <- c(round(rnorm(nsim,mean=rnorm(nsim,mean=6.5,sd=0.1),sd=1),0)) #change the number for mean to make the sites similar or different
site2means <- c(round(rnorm(nsim,mean=rnorm(nsim,mean=5.5,sd=0.1),sd=1),0)) 
P <- 1000 #this gives us a distribution that we expect (long right tail)
#L <- rnorm(n=nsim,mean=site2means,sd=0.1)

#simulate the latencies per individual per site
testdat<-matrix(nrow=nsim,ncol=2)
testdat<-as.data.frame(testdat)
colnames(testdat)<-c("latency","site")
for (i in 1:(nsim/2)){
  L <- exp(site1means) #the linear model for site 1
  latency <- rgampois(1,L,P) #x=is the number of random observations to sample, mean of the gamma distribution=L of the gamma distribution, scale=P
  testdat[i,1]<-latency
  testdat[i,2]<-1 #site 1
}
...
for (j in ((nsim/2)+1):((2*nsim)/2)){
  ...
  testdat[j,1]<-latency
  testdat[j,2]<-2 #site 2
}
#make all non-participatory birds have a 2701
testdat[testdat[,1]>2700,1]<-2701 


#populate the simulated data sheet
sim_manipulationeffects<-c(0,0.1,0.25) #0=no difference between manipulated vs non manipulated birds, 0.1=manipulated birds have a 10% higher probability of solving  locus, etc.
sim_sitedifferences<-c(0,8,10,20) #0.1=compared to core population the edge individuals have a 10% higher latency. alpha mean: 4.6=log of 100 sec (seconds to approach an object), 110s=4.7; log(450) sec = 6.1 mean, log(500) sec = 6.2 mean, 550s=6.3, 600s=6.4; 7.1=log 1200 sec, 1339s=7.2, 1480s=7.3, 1636s=7.4
...
for(sitediff in 1:length(sim_sitedifferences)){
  for(manipeffect in 1:length(sim_manipulationeffects)){
    ...
    for(repetition in 1:10){
nsim = 40 # /2 = n per site when running 2 sites
sim_treat =  sample(0:1,size=nsim,replace=T) #treatment is a categorical variable: manipulated or not

#run the simulated data through the model
simedat <- list(latency = as.integer(testdat[,1]),
            site = as.integer(testdat[,2]),
            treatment = sim_treat
)
simem <- ulam(alist(
