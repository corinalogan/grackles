...
d1 = glmer(round(Lat20cm) ~ (1|BirdID), data = e.CA, family = poisson)
overdisp_fun(d1) #significantly overdispersed
d2 = glmer.nb(round(Lat20cm) ~ (1|BirdID), data = e.CA)
overdisp_fun(d2) #not overdispersed

## Deviation - our dependent variable is overdispersed and thus a Poisson model is not appropriate. Negative binomial not overdispersed with this funtion, but fails data checks below.

# DATA CHECKING
library(DHARMa)
library(lme4)
simulationOutput <- simulateResiduals(fittedModel = lmer(log(Lat20cm) ~ Time + (1|BirdID), data=e.CA), n=250) #250 simulations, but if want higher precision change n>1000
...
testDispersion(simulationOutput) #if under- or over-dispersed, then p-value<0.05, but then check the dispersion parameter and try to determine what in the model could be the cause and address it there, also check for zero inflation
#p=0.90 so not over or under dispersed
...
# p = 0.01, data not normally distributed. 

plot(simulationOutput)#...there should be no pattern in the data points in the right panel
# does not look good

# REPEATABILITY MODELS... in process for CA ExpEnv...
library(MCMCglmm)
# prior = list(R=list(R1=list(V=1,nu=0)), G=list(G1=list(V=1,nu=0)))
# expl <- MCMCglmm( ~  + (1|BirdID), data=e.CA, verbose=F, prior=prior, nitt=13000, thin=10, burnin=3000) 
# summary(expl)
# autocorr(expl$Sol) #Did fixed effects converge? Values after Lag 0 should be < 0.1.
