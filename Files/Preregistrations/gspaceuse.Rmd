...
Traditional studies of animal space use (daily movement patterns within the home range) required spatial and temporal independence of data points for statistical analysis [@swihart1985testing]. However, movement behavior is influenced by internal state, as well as the availability of habitat and resources, which can be distributed non-randomly across the landscape [@nathan2008movement]. Therefore, movement data points are unlikely to meet traditional spatio-temporal independence criteria. Space use behavior also consistently differs among individuals [@hertel2020guide], which indicates that each individual has distinct preferences for how, when, and where to move within its home range. Spatial and temporal autocorrelation (where individuals are repeatedly found in the same locations across time, such that subsequent relocations are predictable based on previous space use) is thus an intrinsic component of space use behavior and eliminating it can reduce biological relevance and obscure relationships with behavioral types [@dray2010exploratory]. Therefore, to investigate the relationship between individual differences in behavioral types and daily space use, it is important to explicitly consider the autocorrelated nature of movement paths by analyzing average step length and turning angle, and correlations among these variables. 
