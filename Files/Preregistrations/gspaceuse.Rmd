...
telem = pts.min[,c(1,2,3,7,11,12)] #only need bird name, date, lat and long, time
#telem$timestamp = paste(telem$Date,telem$Time, sep=' ') #telem format requires date/time merged together for timestamp
telem = telem[,-c(2,3)] #remove other date and time columns
...
for(i in telem){
  ...
  m = ctmm.select(ctmm.fit(telem$i, GUESS))
  hr = akde(telem$i, m)
