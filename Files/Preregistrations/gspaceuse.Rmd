...
From the GPS point locations collected on each individual, we will use a Bayesian model (detailed below) to extract mean and dispersion of step lengths and turning angles for each bird on each daily track, as in @pacheco2019nahua. We will determine if the amount of daily autocorrelation is repeatable within individuals across days by analyzing the distribution of the dispersion parameter for step length and turning angle. A narrow distribution would indicate there is low variability (high repeatability) in the daily movement behaviors of the individual.

Moreover, we will determine if grackles show individual differences in consistent use of habitat by overlaying a 5 x 5m grid array across the landscape.  We will then create matrices describing the number of times a grackle was observed in each cell each day. High autocorrelation among each daily matrix indicates a low-exploring individual that frequents the same spatial locations across days.

If movement behavior and spatial location choice is repeatable, then we will use the average value for each individual to model the relationship with performance at our exploration tasks, and study site location. 
