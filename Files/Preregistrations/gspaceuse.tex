...
for methods) are related to variation in space use (measured via home
...
male's territories (Johnson et al.
...
\KeywordTok{library}\NormalTok{(ctmm)}
...
\CommentTok{### Home range size can only be calculated when a bird has 20 or more relocations}
\CommentTok{### Count the number of relocations for each bird, then exclude individuals with less than 20}
...
\NormalTok{pts.min =}\StringTok{ }\NormalTok{pts.min[}\OperatorTok{-}\KeywordTok{which}\NormalTok{(pts.min}\OperatorTok{$}\NormalTok{count}\OperatorTok{<}\DecValTok{20}\NormalTok{),]}
...
\CommentTok{#### Calculating home range size ####}
\CommentTok{#Turn dataframe into telemetry object}
\NormalTok{telem =}\StringTok{ }\NormalTok{pts.min[,}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{,}\DecValTok{4}\NormalTok{,}\DecValTok{5}\NormalTok{,}\DecValTok{7}\NormalTok{)] }\CommentTok{#only need bird name, date, lat and long, time}
\NormalTok{telem}\OperatorTok{$}\NormalTok{timestamp =}\StringTok{ }\KeywordTok{paste}\NormalTok{(telem}\OperatorTok{$}\NormalTok{Date,telem}\OperatorTok{$}\NormalTok{Time, }\DataTypeTok{sep=}\StringTok{' '}\NormalTok{) }\CommentTok{#telem format requires date/time merged together for timestamp}
\NormalTok{telem =}\StringTok{ }\NormalTok{telem[,}\OperatorTok{-}\KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{,}\DecValTok{5}\NormalTok{)] }\CommentTok{#remove other date and time columns}
\KeywordTok{colnames}\NormalTok{(telem) =}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"individual.local.identifier"}\NormalTok{,}\StringTok{"location.lat"}\NormalTok{,}\StringTok{"location.long"}\NormalTok{,}\StringTok{"timestamp"}\NormalTok{) }\CommentTok{#use required column names}
\NormalTok{telem =}\StringTok{ }\KeywordTok{as.telemetry}\NormalTok{(telem, }\DataTypeTok{timeformat =} \StringTok{"%Y-%m-%d %H:%M:%S"}\NormalTok{,}\DataTypeTok{timezone =} \StringTok{""}\NormalTok{)}

\ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(telem))\{}
\NormalTok{  GUESS =}\StringTok{ }\KeywordTok{ctmm.guess}\NormalTok{(telem}\OperatorTok{$}\NormalTok{i, }\DataTypeTok{interactive =} \OtherTok{FALSE}\NormalTok{)}
\NormalTok{  m =}\StringTok{ }\KeywordTok{ctmm.fit}\NormalTok{(telem}\OperatorTok{$}\NormalTok{i, GUESS)}
\NormalTok{  hr =}\StringTok{ }\KeywordTok{akde}\NormalTok{(telem}\OperatorTok{$}\NormalTok{i, m)}
\NormalTok{  est[i] =}\StringTok{ }\KeywordTok{summary}\NormalTok{(hr)}\OperatorTok{$}\NormalTok{CI[}\DecValTok{2}\NormalTok{]}
\NormalTok{  results =}\StringTok{ }\KeywordTok{rbind}\NormalTok{(results,est[i])}
\NormalTok{\} }


\CommentTok{### Normalize data for repeatability and regression models }
\KeywordTok{hist}\NormalTok{(}\KeywordTok{log}\NormalTok{(hr}\OperatorTok{$}\NormalTok{area)) }

\CommentTok{### Repeatability of home range in breeding and non-breeding seasons}
\NormalTok{hr_rpt =}\StringTok{ }\KeywordTok{rpt}\NormalTok{(}\KeywordTok{log}\NormalTok{(area) }\OperatorTok{~}\StringTok{ }\NormalTok{(}\DecValTok{1}\OperatorTok{|}\NormalTok{BirdID), }\DataTypeTok{grname =} \StringTok{"BirdID"}\NormalTok{, }\DataTypeTok{data =}\NormalTok{ hr, }\DataTypeTok{datatype =} \StringTok{"Gaussian"}\NormalTok{, }\DataTypeTok{nboot =} \DecValTok{500}\NormalTok{, }\DataTypeTok{npermut =} \DecValTok{500}\NormalTok{)}
\KeywordTok{summary}\NormalTok{(hr_rpt)}

\CommentTok{### Make data sheet with the experimental exploration measures combined with the home range measures}
...
\NormalTok{ heading[s]<-(heading[s}\DecValTok{-1}\NormalTok{]}\OperatorTok{+}\NormalTok{Theta)}\OperatorTok{%%}\DecValTok{360}                                          \CommentTok{# Store new heading. Note that the %% is the mod operation to wrap around if needed}
...
\NormalTok{ loc.x[s]<-loc.x[s}\DecValTok{-1}\NormalTok{]}\OperatorTok{+}\NormalTok{xnew                                                      }\CommentTok{# Make the new x and y pair}
\NormalTok{ loc.y[s]<-loc.y[s}\DecValTok{-1}\NormalTok{]}\OperatorTok{+}\NormalTok{ynew                                                      }\CommentTok{#}
...
\NormalTok{ Dist[i]<-}\StringTok{ }\KeywordTok{dist}\NormalTok{(X[i],X[i}\DecValTok{-1}\NormalTok{],Y[i],Y[i}\DecValTok{-1}\NormalTok{]);}
\NormalTok{ ang1[i]<-}\StringTok{ }\KeywordTok{ang}\NormalTok{(X[i],X[i}\DecValTok{-1}\NormalTok{],Y[i],Y[i}\DecValTok{-1}\NormalTok{])}
 \ControlFlowTok{if}\NormalTok{(i}\OperatorTok{>}\DecValTok{3}\NormalTok{)\{}
\NormalTok{ angdif[i]<-}\StringTok{ }\KeywordTok{ang.dif}\NormalTok{(ang1[i],ang1[i}\DecValTok{-1}\NormalTok{])}
\NormalTok{         \}\}    }
...
\CommentTok{#m1 = lm(log(area) ~ ExpObj + ExpEnv + Sex + History + Breeding + Feeding + Condition + Group + Season, data = data)}
\KeywordTok{hist}\NormalTok{(m1}\OperatorTok{$}\NormalTok{resid)}
\KeywordTok{summary}\NormalTok{(m1)}

\CommentTok{# Step length }
\CommentTok{#m2 = lm(log(std_step)) ~ ExpObj + ExpEnv + Sex + History + Breeding + Feeding + Condition + Group + Season, data = data)}
\KeywordTok{hist}\NormalTok{(m2}\OperatorTok{$}\NormalTok{resid)}
\KeywordTok{summary}\NormalTok{(m2)}

\CommentTok{# Turning angle}
\CommentTok{#m3 = lm(log(std_angle)) ~ ExpObj + ExpEnv + Sex + History + Breeding + Feeding + Condition + Group + Season, data = data)}
\KeywordTok{hist}\NormalTok{(H3}\OperatorTok{$}\NormalTok{resid)}
\KeywordTok{summary}\NormalTok{(m3)}

\CommentTok{# Spatial preferences}
\CommentTok{#m4 = lm(log(loc_pref)) ~ ExpObj + ExpEnv + Sex + History + Breeding + Feeding + Condition + Group + Season, data = data)}
\KeywordTok{hist}\NormalTok{(H4}\OperatorTok{$}\NormalTok{resid)}
\KeywordTok{summary}\NormalTok{(m4)}
\end{Highlighting}
\end{Shaded}

\hypertarget{h2-p2---space-use-behaviors-vary-among-populations-across-the-range}{%
\paragraph{H2: P2 - Space use behaviors vary among populations across
the
range}\label{h2-p2---space-use-behaviors-vary-among-populations-across-the-range}}
...
\CommentTok{# Home range }
\CommentTok{#m1 = lm(log(area) ~ Site + Sex + History + Season, data = data)}
\KeywordTok{hist}\NormalTok{(m1}\OperatorTok{$}\NormalTok{resid)}
\KeywordTok{summary}\NormalTok{(m1)}

\CommentTok{# Step length}
\CommentTok{#m2 = lm(log(std_step)) ~ Site + Sex + History + Season, data = data)}
\KeywordTok{hist}\NormalTok{(m2}\OperatorTok{$}\NormalTok{resid)}
\KeywordTok{summary}\NormalTok{(m2)}

\CommentTok{# Turning angle}
\CommentTok{#m3 = lm(log(std_angle)) ~ Site + Sex + History + Season, data = data)}
\KeywordTok{hist}\NormalTok{(m3}\OperatorTok{$}\NormalTok{resid)}
\KeywordTok{summary}\NormalTok{(m3)}

\CommentTok{# Spatial preference}
\CommentTok{#m4 = lm(log(loc_pref)) ~ Site + Sex + History + Season, data = data)}
