...
hist(diversity$AvgLatencyAttemptNewLoci, xlab="Latency to attempt a new locus (s)", main="Actual Data")
hist(diversity$AvgLatencySolveNewLoci, xlab="Latency to solve a new locus (s)", main="Actual Data")
...
# DATA CHECKING: Checking the residuals using the R package DHARMa (https://cran.r-project.org/web/packages/DHARMa/vignettes/DHARMa.html)
# NOTE: DHARMa doesn't support MCMCglmm, therefore we are using the closest supported model: glmerMod from the R package lme4
library(DHARMa)
simulationOutput <- simulateResiduals(fittedModel = glmerMod(TrialsToReverse ~ ReverseNumber + (1|ID) + (1|Batch)), n=250) #250 simulations, but if want higher precision change n>1000
...
plot(simulationOutput) #QQ plot: want points on the red line, residual vs predicted lines should match each other. In the latter, red stars=data that are outside the range of simulated data
plotResiduals(ReverseNumber, simulationOutput$scaledResiduals) #plot the residuals against other predictors
```
...
**P2 alternative 2: additional analysis: latency and motor diversity** A Generalized Linear Mixed Model (GLMM; MCMCglmm function, MCMCglmm package; [@hadfieldMCMCglmmpackage]) will be used with a Poisson distribution and log link using 13,000 iterations with a thinning interval of 10, a burnin of 3,000, and minimal priors (V=1, nu=0) [@hadfield2014coursenotes]. We will ensure the GLMM shows acceptable convergence (lag time autocorrelation values <0.01; [@hadfieldMCMCglmmpackage]), and adjust parameters if necessary. We will determine whether an independent variable had an effect or not using the Estimate in the full model.
...
prior = list(R = list(R1 = list(V = 1, nu = 0), R2 = list(V = 1, nu = 0)), G = list(G1 = list(V = 1, 
	    nu = 0)))
rm <- MCMCglmm(log(LatencySolveNewLoci) ~ Condition * ReversalNumber * TrialsToReverseT * TrialsToReverse, random = ~ID, 
