...
This means that, with our sample size of 32, we have a 71% chance of detecting a medium effect (approximated at f^2^=0.15 by @cohen1988statistical). A similar study examining the decline of error rates in serial reversals in three corvid species found a large effect size when analyzing their data (@bond_serial_2007), and they had the power to detect such an effect (see the G*Power analysis we performed on their study in the review history of this preregistration when it is available at PCI Ecology). Therefore, we might expect a similarly large effect in our study, and we should be able to detect it according to this power analysis.
