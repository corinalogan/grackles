...
res_df2 = merge(res_df2, d_all, by = c("Bird.Name","Site"))
...
model <- glmmTMB(mean ~ Site + 
                      Sex + Season2 + History,
                 data = my_data, 
                 family = beta_family())
simulationOutput <- simulateResiduals(fittedModel = betareg(mean ~ Site + #note that we are not including population density as preregistered because it is significantly correlated with Site
