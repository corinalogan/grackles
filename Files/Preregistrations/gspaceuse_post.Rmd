...
hr_total = merge(hr_exp, covs, by = c("BirdID","Site"), all = T)
...
hr_red = hr_red[-which(is.na(hr_red$Group.Size)),] #41 grackles with all data

```

```{r POST H1-home range and exploration, eval=FALSE, include=FALSE}
#### H1 - Does home range size relate to exploration? ####
# load data sheet created in previous R chunk
HR <- read.csv(url("https://raw.githubusercontent.com/corinalogan/grackles/541c762fba2e8cabc70f19b3778057d498f22e9a/Files/Preregistrations/gspaceuse_HRdataSheet.csv"), header=T, sep=",", stringsAsFactors=F)

# Check for significant correlations among the covariates
tmp = HR[,-c(1,2,3,7,13)] # only numeric variables
...
# The two exploration variables are significantly correlated (r = -0.58), so only include one in the model at a time
# Also, Group size is correlated with SMI and Food Sites; and Food sites is also correlated with History and hr
# Remove Group size and History from models and test against HR separately


##### DATA CHECKING sites combined #####
library(DHARMa)
library(lme4)
simulationOutput <- simulateResiduals(fittedModel = lm(log(hr) ~ DurationNear + Sex + Breeding.Sites + Food.Sites + SMI + Season, data=HR), n=250) #250 simulations, but if want higher precision change n>1000
...
library(MCMCglmm)
m1.lat.red = MCMCglmm(log(hr) ~ LatencyFirstLand + Site + Sex + Breeding.Sites + Food.Sites + SMI + Season, family="gaussian", data=HR, verbose=F, nitt=100000, thin=50, burnin=8000)
...
m1.dur.red = MCMCglmm(log(hr) ~ DurationNear + Site + Sex + Breeding.Sites + Food.Sites + SMI + Season, family="gaussian", data=HR, verbose=F, nitt=100000, thin=50, burnin=8000)
...
##### Analyze Sites separately as home ranges look very different in each place #####
## AZ ##
tmpAZ = HR[which(HR$Site == "AZ"),-c(1:3,7,13)]
AZcorr_matrix <- cor(tmpAZ)
AZpvals <- cor_pmat(tmpAZ)
AZcorr = ggcorrplot(AZcorr_matrix, 
           p.mat = AZpvals,
           sig.level = 0.05,
           insig = "blank",
           hc.order = TRUE,           # Reorder variables using hierarchical clustering
...
# Exploration variables correlated; and SMI is correlated with Group.size; test separately

AZm1.lat.red = MCMCglmm(log(hr) ~ LatencyFirstLand + Sex + History + Breeding.Sites + Food.Sites + SMI + Season, family="gaussian", data=HR[which(HR$Site == "AZ"),], verbose=F, nitt=100000, thin=50, burnin=8000)
autocorr(AZm1.lat.red$Sol) #Did fixed effects converge? Values after Lag 0 should be < 0.1
autocorr(AZm1.lat.red$VCV) #Did error converge? Yes
summary(AZm1.lat.red) # No significant effects

AZm1.lat.red2 = MCMCglmm(log(hr) ~ LatencyFirstLand + Sex + History + Breeding.Sites + Food.Sites + Group.Size + Season, family="gaussian", data=HR[which(HR$Site == "AZ"),], verbose=F, nitt=100000, thin=50, burnin=8000)
autocorr(AZm1.lat.red2$Sol) #Did fixed effects converge? Values after Lag 0 should be < 0.1
autocorr(AZm1.lat.red2$VCV) #Did error converge? Yes
summary(AZm1.lat.red2) # No significant effects

AZm1.dur.red = MCMCglmm(log(hr) ~ DurationNear + Sex + History + Breeding.Sites + Food.Sites + SMI + Season, family="gaussian", data=HR, verbose=F, nitt=100000, thin=50, burnin=8000)
autocorr(AZm1.dur.red$Sol) #Did fixed effects converge? Values after Lag 0 should be < 0.1
autocorr(AZm1.dur.red$VCV) #Did error converge? Yes
summary(AZm1.dur.red) # More food sites = larger home range 

AZm1.dur.red2 = MCMCglmm(log(hr) ~ DurationNear + Sex + History + Breeding.Sites + Food.Sites + Group.Size + Season, family="gaussian", data=HR, verbose=F, nitt=100000, thin=50, burnin=8000)
autocorr(AZm1.dur.red2$Sol) #Did fixed effects converge? Values after Lag 0 should be < 0.1
autocorr(AZm1.dur.red2$VCV) #Did error converge? Yes
summary(AZm1.dur.red2) # More food sites = larger home range


## CA ##
tmpCA = HR[which(HR$Site == "CA"),-c(1:3,7,13)]
CAcorr_matrix <- cor(tmpCA)
CApvals <- cor_pmat(tmpCA)
CAcorr = ggcorrplot(CAcorr_matrix, 
           p.mat = CApvals,
           sig.level = 0.05,
           insig = "blank",
           hc.order = TRUE,           # Reorder variables using hierarchical clustering
...
# Exploration variables correlated; also SMI is correlated with Group.size & Food sites; test separately

CAm1.lat.red = MCMCglmm(log(hr) ~ LatencyFirstLand + Sex + History + Breeding.Sites + SMI + Season, family="gaussian", data=HR[which(HR$Site == "CA"),], verbose=F, nitt=100000, thin=50, burnin=8000)
autocorr(CAm1.lat.red$Sol) #Did fixed effects converge? Values after Lag 0 should be < 0.1
autocorr(CAm1.lat.red$VCV) #Did error converge? Yes
summary(CAm1.lat.red) # No significant effects

CAm1.lat.red2 = MCMCglmm(log(hr) ~ LatencyFirstLand + Sex + History + Breeding.Sites + Group.Size + Food.Sites + Season, family="gaussian", data=HR[which(HR$Site == "CA"),], verbose=F, nitt=100000, thin=50, burnin=8000)
autocorr(CAm1.lat.red2$Sol) #Did fixed effects converge? Values after Lag 0 should be < 0.1
autocorr(CAm1.lat.red2$VCV) #Did error converge? Yes
summary(CAm1.lat.red2) # No significant effects

CAm1.dur.red = MCMCglmm(log(hr) ~ DurationNear + Sex + History + Breeding.Sites + SMI + Season, family="gaussian", data=HR, verbose=F, nitt=100000, thin=50, burnin=8000)
autocorr(CAm1.dur.red$Sol) #Did fixed effects converge? Values after Lag 0 should be < 0.1
autocorr(CAm1.dur.red$VCV) #Did error converge? Yes
summary(CAm1.dur.red) # No significant effects

CAm1.dur.red2 = MCMCglmm(log(hr) ~ DurationNear + Sex + History + Breeding.Sites + Group.Size + Food.Sites + Season, family="gaussian", data=HR[which(HR$Site == "CA"),], verbose=F, nitt=100000, thin=50, burnin=8000)
autocorr(CAm1.dur.red2$Sol) #Did fixed effects converge? Values after Lag 0 should be < 0.1
autocorr(CAm1.dur.red2$VCV) #Did error converge? Yes
summary(CAm1.dur.red2) # No significant effects
```
