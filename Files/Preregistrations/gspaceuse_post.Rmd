...
tmp = hr_red[,-c(1,2,6,12)] # only numeric variables
...
# The two exploration variables are significantly correlated (r = -0.57), so only include one in the model at a time

##### DATA CHECKING sites combined #####
library(DHARMa)
library(lme4)
simulationOutput <- simulateResiduals(fittedModel = lm(log(hr) ~ DurationNear + Sex + Season + Breeding.Sites + Food.Sites + Group.Size + History + SMI, data=HR), n=250) 
...
library(MCMCglmm)
m1.lat.red = MCMCglmm(log(hr) ~ scale(LatencyFirstLand, center = TRUE, scale = TRUE) + Site + Sex + Season + Breeding.Sites + Food.Sites, family="gaussian", data=HR, verbose=F, nitt=100000, thin=50, burnin=8000)
...
m1.dur.red = MCMCglmm(log(hr) ~ scale(DurationNear, center = TRUE, scale = TRUE) + Site + Sex + Season + Breeding.Sites + Food.Sites, family="gaussian", data=HR, verbose=F, nitt=200000, thin=50, burnin=8000)
...
### Autocorrelation of step length
```{r POST Step length and exploration, echo=FALSE, message=FALSE, warning=FALSE}
...
res_tot2$Pred = factor(res_tot2$Pred, levels = c("Duration","Season2","Food.Sites","Breeding.Sites",
                      ...
                      labels = c("Novel environment exploration","Season (Breeding)",
