...
hr_red$hr = as.numeric(hr_red$hr)


##### DATA CHECKING sites combined #####
library(DHARMa)
library(lme4)
simulationOutput <- simulateResiduals(fittedModel = lm(log(hr) ~ LatencyFirstLand + DurationNear + Sex + History + Breeding.Sites + Food.Sites + SMI + Group.Size + Season, data=hr_red), n=250) #250 simulations, but if want higher precision change n>1000
plot(simulationOutput$scaledResiduals) #Expect a flat distribution 
testDispersion(simulationOutput) #if under- or over-dispersed, then p-value<0.05, but then check the dispersion parameter and try to determine what in the model could be the cause and address it there, also check for zero inflation
#p=0.26 so not over or under dispersed
...
library(MCMCglmm)
m1 = MCMCglmm(log(hr) ~ LatencyFirstLand + DurationNear + Sex + History + Breeding.Sites + Food.Sites + SMI + Group.Size + Season, family="gaussian", data=hr_red, verbose=F, nitt=13000, thin=10, burnin=3000)
...
library(olsrr) # Reduce variables to make sure we aren't overfitting the model with our sample size
m2 = lm(log(hr_red$hr) ~ Site + LatencyFirstLand + DurationNear + Sex + History + Breeding.Sites + Food.Sites + SMI + Group.Size + Season, data = hr_red) #function requires lm class
ols_step_backward_p(m2, details = T)

m2_red = lm(log(hr)~ Site + LatencyFirstLand + Sex + Food.Sites + Group.Size, data = hr_red)
summary(m2_red) # Group.Size significant

##### Analyze Sites separately as home ranges look very different in each place #####
AZhr_red = hr_red[which(hr_red$Site == "AZ"),]
m2AZ = lm(log(AZhr_red$hr) ~ LatencyFirstLand + DurationNear + Sex + History + Breeding.Sites + Food.Sites + SMI + Group.Size + Season, data = AZhr_red) 
ols_step_backward_p(m2AZ, details = T)

m2AZ_red = lm(log(AZhr_red$hr) ~ LatencyFirstLand + DurationNear + Sex + Group.Size, data = AZhr_red) 
summary(m2AZ_red) # Group.Size and LatencyFirstLand significant
# As LatencyFirstLand decreases, hr size increases; as Group.Size increases, hr increases

CAhr_red = hr_red[which(hr_red$Site == "CA"),]
m2CA = lm(log(CAhr_red$hr) ~ LatencyFirstLand + DurationNear + Sex + History + Breeding.Sites + Food.Sites + SMI + Group.Size + Season, data = CAhr_red) 
ols_step_backward_p(m2CA, details = T)

m2CA_red = lm(log(CAhr_red$hr) ~ Food.Sites + Group.Size, data = CAhr_red) 
summary(m2CA_red) # Group.Size significant
