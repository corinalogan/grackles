---
title: Space use behavior is unrelated to exploration in a species that is rapidly
  expanding its geographic range
author:
- '[McCune Kelsey B](https://www.kelseymccune.com/)^1^*'
- Ross Cody^2^
- LeGrande-Rolls Christa^2^
- Folsom Melissa^2^
- Bergeron Luisa^1^
- '[Logan Corina J](http://CorinaLogan.com)^2^'
date: "`r Sys.Date()`"
output:
  pdf_document:
    keep_tex: yes
...
# ABSTRACT
Species range expansion could be facilitated by consistent behavioral differences between individuals on the range edge and those in other parts of the range. Movement behaviors in particular are thought to be important for expanding and invasive species. There is evidence for a relationship between individual exploratory traits and dispersal (the permanent movement an individual makes from its birth site to the place where it reproduces), but it is still unknown whether individual differences in exploration relate to adult daily movement patterns within the home range (“space use”). Evidence suggests that daily space use behavior can vary consistently among individuals, but no studies have examined the relationship between space use behavior and measures of exploration, or space use behavior in different populations along an expanding range. Here we studied great-tailed grackles (*Quiscalus mexicanus*), species that is rapidly expanding their geographic range, from 2 populations across the current range - Arizona (middle of the northern expanding edge), and northern California (near the northern edge of their range). We evaluated whether performance on an exploration task in captivity related to subsequent space use behavior in the wild measured using home range size, the autocorrelation of step length (distance between two sequential observations) and turning angle for each individual over time, and the repeatability of each individual's occurrence in particular geographic locations. 
# edit below with results #
We found that ... These results indicate that individual differences in exploration could be a contributing mechanism explaining variation in space use in the wild, and that population proximity to the edge of the range can explain variation in some aspects of daily space use patterns. 
If space use behavior correlates with measures of exploration, then space use data could be used to inform conservation management strategies (e.g. identify which individuals are likely to remain in new or restored habitat after a translocation [@may2016predicting]) in species where it is not logistically feasible to *a priori* measure exploration in captivity. If daily space use patterns differ in populations at different locations on the expanding range, then future studies could investigate whether space use and exploration are coupled with other traits such as long-distance natal dispersal to form an “invasion syndrome” [@chapple2012can].

# INTRODUCTION

Problematic range expansions of invasive species are occurring across the globe. However, not all species that invade novel areas outside of their traditional range become established [@chapple2012can; @hayes2008there] and it is still unclear what characteristics facilitate successful invasions [@fogarty2011social]. These characteristics are important for predicting potential invasions, especially since invasive species are implicated as a leading cause of biodiversity loss [@butchart2010global; @clavero2005invasive]. One hypothesis is that the composition of behavioral types within species at the invasion front might facilitate geographic range expansion [@carere2013animal] because certain individuals may possess traits such as exploration and aggressiveness that make them more likely to succeed when venturing into new habitats and competing with heterospecifics [@cote2010personality]. Evidence is already accumulating that links consistent individual differences in behavioral types and invasion success. For example, Duckworth and colleagues [@duckworth2007coupling] found more aggressive behavior in western bluebird males on the range edge, suggesting that range expansion was facilitated by aggressive males dispersing further and displacing less aggressive mountain bluebirds. Additionally, invasive cane toads on the invasion front in Australia spent longer in the dispersal movement mode and moved greater distances than toads tracked several years later in the same location [@lindstrom2013rapid]. 

Within-species variation in the ability (movement) and motivation (exploratory tendency) to encounter conspecifics, novel foods, and novel food sources could be a limiting factor in successful species range expansions [@spiegel2016feedback]. In novel areas, the occurrence of conspecifics, food, predators and other environmental factors may not be as easily detectable or recognizable, and may be distributed differently across the landscape than in core areas of the range. Although individuals with exploratory phenotypes may be more successful at colonizing new habitat, exploratory individuals are also at higher risk of predation [@stuber2013slow], and could be less likely to find local food sources [@van2009personality]. Consequently, for establishment in new areas, individuals that exhibit a range of exploratory behavior may be needed, and the interaction between space use and exploratory tendency is likely important for finding novel foods and food sources. Additionally, while dispersal [the permanent movement an individual makes from its birth site to the place where it reproduces; @greenwood1982natal] is necessary to initially invade the novel habitat, subsequent daily space use could determine establishment success. For example, on the range edge conspecific density might be lower and individuals may be able to maintain larger home range sizes, or may use space differently due to novel interspecific competitive interactions [@bubb2006movement], or maintaining larger home range sizes. However, current research on invasion success and movement behavior focuses heavily on dispersal, and while dispersal and exploratory tendencies have been shown to be associated [@cote2010personality], we do not know how exploratory tendency influences space use patterns in the daily lives of invading individuals.
...
**H2: Space use behavior will vary among grackles from our two study populations located along different points in the geographic range of this species (middle of expansion, and range edge; Table 1). These populations are theoretically connected, however actually moving between two of our field sites within a few grackle lifespans is unlikely due to the large distances between field sites and geographic barriers (the Sierra Nevada and Sierra Madre mountain ranges).**

**Prediction 2:** Home range size will increase, autocorrelation of step lengths and turning angles will decrease (i.e. grackle movement behavior will be less predictable), and grackles will use a greater variety of spatial locations as the geographic distance from the center of the range increases. Specifically, the grackles sampled from our site on the edge of the range (northern California), will have larger overall home range sizes, exhibit more variety in step lengths and turning angles, and use a greater variety of spatial locations than the sample of grackles in the middle of the range (Arizona). Grackles in the sample from the middle of the expanding range will be intermediate in space use (Arizona). Such population differences in space use behavior may relate to range expansion because some of the individuals on the leading edge of the range may use more space and move longer distances [@duckworth2007coupling]. However, larger-scale sampling of grackle groups across the strata of the expansion front and core range would be needed to more robustly validate the hypothesis that our cross-site differences are indicative of a broader pattern driven by the location of the expansion front.

**Prediction 2 alternative 1:** Grackles sampled on the edge of the range will have smaller overall home range sizes, high autocorrelation in step length and turning angle (i.e. movement behavior will be more predictable), and consistently use the same spatial locations compared to grackles sampled in the middle of the current range. This would suggest that suitable habitat may be distributed in small patches, that novel habitats at the edge of the range may have high predation on grackles that use more space, and/or that individual grackles specialize on certain novel habitat types that are patchily distributed.

**Prediction 2 alternative 2:** We will find no difference across the two populations in the space use behavior of the grackles sampled. This would suggest that, on average, all grackles may use the same amount of space, or that there is a similar distribution of individual differences in space use in each population. Alternatively, it could indicate that we did not detect differences because we measured adults rather than juveniles. Grackles sampled in different populations may converge on similar space use behavior during development, or juvenile grackles may disperse further on the edge of the range. However, we are not able to detect these differences with our data, which is primarily from adults.


### DEVIATIONS FROM THE PREREGISTRATION

1) Due to the COVID-19 pandemic, we were unable to travel to Central America to measure space use and exploratory behavior of grackles in the endemic portion of the range.

2) We modeled only step length - rather than both step length and turning angle - as an outcome for assessing autocorrelation of space use behavior. The preregistered model of search dynamics introduced in @ross2018general [tested in @pacheco2019nahua; @ross2018evidence] relies on equal-interval GPS points over short time-steps. Due to constraints in the field, GPS data could not be collected using exactly equal-interval track points. Although we instead have GPS data taken at uneven, but still fairly uniform intervals. As such, autocorrelation in step length can still be modeled by normalizing the distance between GPS points by the time between GPS points, to yield a search velocity. However, calculation of standardized turning angles in this way was not possible. 

3) We discovered in our investigation of grackle personality traits [@mccune2025behavioral] that performance on the novel object test was not repeatable. This indicates that this particular test was not a good measure of the personality trait exploration. As such, here we only consider grackle exploratory tendency as the performance on the novel environment test.	

4) Because the population density of California grackles was significantly less than the density of grackles in Arizona, we could not include population density in the models for H2 when the categorical variable for Site was already included. This is because these two correlated variables accounted for the same variance.
...
Great-tailed grackles (n = 22 in AZ, n = 22 in CA) were caught from the wild using a variety of methods (mist nets, walk-in traps, and bow nets) and brought into large aviaries at each site. We prioritized capturing adult individuals as cognitive and behavioral traits are often not fully developed in hatch year birds [i.e., @zucca2007piagetian]. We identified grackles as adults using eye color, where hatch year birds have brown eyes, but second year and older birds have yellow to white eyes. While it is possible that cognitive traits continue to develop in second year and older birds, it is impossible to distinguish grackle age after the bird’s first year [@johnson2001great]. We also attempted to balance subjects by sex. Subjects spent up to six months in an aviary while they participated in behavioral choice tests and individual differences assays, including measures of exploration in captivity [see below], as part of other research projects by this lab.   

## Space use 
Before the grackles were released from the aviaries, they were fitted with a VHF radio tag (Lotek PipLL model Ag386, Advanced Telemetry Systems model A2455, or Holohil Systems Ltd. model BD-2) attached to a leg loop harness [methods as in @rappole1991new] so that we could track space use behavior in the wild. We searched for each tagged grackle multiple times per week. Once found, the experimenter followed the focal individual for approximately 20 - 90 minutes, using the iPhone application GPX Tracker [@merlos_2014] to record a GPS point at the bird’s location every one minute, regardless of whether the bird moved [@cushman2005elephants]. Researchers only approached to a distance that did not affect the grackle’s behavior. If the grackle alarm called or oriented towards the researcher (indicating the researcher's presence affected the grackle's behavior), all tracking on that individual was stopped for the day. To ensure we captured all locations that the individual visits and not just those where they are most easily seen and followed, tagged grackles that move out of sight during tracking were searched for with telemetry until they are found again. 

We attempted to balance the collection of space use data across morning and afternoon time periods for all grackles. We hoped to track the same individuals across the breeding and nonbreeding seasons. However, the leg loop harnesses often degraded and fell off after 1 - 4 months. Therefore, we have few data points on the same individuals in both seasons. We excluded GPS points collected outside of “normal daily activities” [@calenge2011home], for example mobbing a predator, or points before sunrise or after sunset when grackles are at the roost. Outside of these circumstances, we included all GPS points in our analysis of space use behavior. 

### Space use dependent variables
We quantified grackle space use behavior with three metrics: home range size, autocorrelation of step length, and spatial location preference. We were unable to test the fourth metric that we preregistered, autocorrelation of turning angle (see Deviations section).

#### Home range size
First, we used a traditional measure of space use that controls spatial and temporal autocorrelation in relocations, home range size. For each grackle that had more than 20 GPS point locations [@noonan2019comprehensive], we calculated home range size using the akde function in the R packages ctmm [@calabrese2016ctmm] and sf [@pebesma2018simple]. 

#### Autocorrelation of step length
Secondly, to explore the behavioral relevance of autocorrelation in space use, we quantified small scale movement decisions by extracting step length and turning angle from the tracking data. Step length is the geographic distance between two sequential GPS points taken at 1-minute intervals, while turning angle is the angular deviation from a straight line path among three sequential GPS points. We were interested in both the average step length per time, as well as the variation in step length per time, since birds with larger variances in step lengths per time tend to show Lévy-like movement, and thus necessarily engage in broader space use by making large spatial displacements with appreciable frequency [@reynolds2009levy]. As such, we calculated the mean and standard deviation of step length per time vectors for each bird. The total number of time-steps used to calculate the mean and standard deviation in step length per time varied across birds. Step length per time was log-transformed and we dropped a few estimates that were implausibly high, likely the result of GPS resolution errors.

#### Spatial location preference
To model temporal consistency in spatial location preference, we used a Dirichlet mixture model [@douma2019analysing; @wang2021bayesian]. We began with the sample space over which grackles were observed to forage. At the California site, this is a 1.3 × 1.9 km rectangle, and at the Arizona site, this is a 2.4 × 1.6 km rectangle. For each site, we divided the sample space into a list of G grid-cells. For the California site, the space is divided into a 58 × 58 set of grid cells, which leads to an average grid size of about 22 × 33 m and a grid density of 1,362 grid cells per km2. For the Arizona site, the sample space is divided into a 71 × 71 set of grid cells, which leads to an average grid size of about 23 × 34 m and a grid density of 1,312 grid cells per km2. By choosing the number of subdivisions to be 58 and 71 respectively, we equalize the scale of inference at both sites. We modeled temporal autocorrelation in space use by studying the bird-specific time-series of foraging (and other movement) behavior. Let F[b,d,g] be the count of times that bird b was observed in grid-cell g on day d. For any day after the first day of continuous observations, we can model the entire G-vector, F[b,d,1:G], using a multinomial regression [@de2013general] of the form:  
              F[b,d,1:G] ∼ Multinomial(Θ[b,d,1:G])	                      (1)
where:					
              Θ[b,d,1:G] = (1 − α[b])θ[1:G] + α[b]F[b,d−1,1:G] 						(2)
where:				
...
Eq. 1 shows that we model the distribution of space-use as a multinomial outcome with prob- ability vector Θ[b,d,1:G]. Eq. 2 then shows how Θ[b,d,1:G] is constructed. Θ[b,d,1:G] is generated as a convex combination of two G-vectors: θ[1:G] is a vector giving the “suitability” or “attractiveness” of each grid-cells to grackles in general, and F[b,d−1,1:G], a vector giving the fraction of time bird b spent in each and every grid-cell on day d−1 (F is just a normalized version of F). The mixing rate of these two components, the suitability component and the 1-day lag component, is controlled by a bird-specific parameter, α[b]. Eq. 3 shows that α[b] is constructed using a simple logistic regression model, with an intercept parameter, β0, slope parameters, βp, bird-specific covariates, Z[p,b], and finally, a bird specific random effect, φ[b]. This regression equation allows us to study how exploratory different birds are: birds with a large α[b] are less exploratory in that their space use on day d is well predicted by their space-use on day d − 1. While birds with a small α[b] are more exploratory, in that they are more likely to visit all grid-cells (in proportion  to their suitability) regardless of their space-use distribution on the previous day. Eqs. 4–7 show that we used weak, non-informative priors. 

To understand spatial location preferences, we determined the rectangles of smallest area that could enclose the maximum number of data-points. The rectangle for the Arizona site, captured 16,618 out of 16,771 GPS points (99.087%) within our sample area (Fig. 1a). For the California site, the rectangle captured 18,717 out of 21,314 GPS points (87.815%) within our sample area (Fig. 1b). The slightly lower fraction of data inclusion in the California site was due to some GPS tracks falling substantially outside of the central study area (see Fig. 1b); for computational reasons, it was impossible to extend the boundaries of our study area to capture all such points, while maintaining a reasonable grid-cell resolution. By focusing on a smaller central region where the bulk of GPS points were observed (Fig. 2), we are able to maintain computational tractability and fine-scale grid-cell resolution, at the cost of potentially underestimating the degree to which some birds explored  wider geographic regions. Geographic boundaries of the in-sample region were selected before any models were fit to the data. 	

Once geographic boundaries were selected, we then used 2-dimensional binning with the “ash”  package [@scott2015ash] in R to generate outcome data unique to birds and days. For the Arizona site, the sample space is divided into a 71 × 71 set of grid cells, and the outcome data for each bird on each day is the number of recorded GPS points inside each of the 5,041 grid-cells. For the California site, the sample space is divided into a 58 × 58 set of grid cells, and the outcome data for each bird on each day is the number of recorded GPS points inside each of the 3,364 grid-cells. Figure 3 shows the overall log density distribution of the outcome data at each site. 			

Models were fit using R [@rcoreteam] and Stan [@carpenter2017stan] through the “RStan” package [@rstan]. Model fit was assessed using the effective sample size and r statistics. Models were also quality-checked by creating simulated data under the data generating process described by Eqs. 1-7 in R, and then using the Stan model to recover the true generative parameters: see tutorial at https://github.com/ctross/grackleator for  details.


## Predictor variables

### Exploration
We used data collected as part of another investigation [@mccune2025behavioral] to quantify the personality trait exploration. We adapted commonly used methods to test exploratory tendency of the grackles that are temporarily held in aviaries. Exploratory tendency was measured as the latency to approach and the duration spent in proximity (within 20cm) of a novel environment set within the familiar environment of their aviary. Exploration assays occur twice for each bird: once near the beginning of their aviary time and once again approximately 6 weeks later. In this previous investigation [@mccune2025behavioral], we also validated that performance on the exploration assays reflected exploration rather than boldness, and that performance was repeatable across time.

### Additional covariates
We included additional predictor variables for grackle sex and whether the data were collected in the breeding or non-breeding season. We also measured energetic condition [@berens2019condition] to account for differences in the physiological mobility that may limit an individual’s space use behaviors [@nathan2008movement]. We included the number of days individuals spent in the aviaries to control for the impact of captivity on movement in the wild. Similarly, we included the maximum group size the focal grackle was observed with during behavioral observations as part of a separate investigation [@logan2025flexforpcj] to account for the impact of variation in social behavior on space use. We also tested whether time spent in captivity altered the social behavior of grackles when subsequently released into the wild by testing maximum group size observed across each individual’s focal follows as a function of the individual’s captivity history (the number of days the individual was temporarily held in the aviaries before data collection on space use began). We measured habitat characteristics at each site such as human food sources and available breeding habitat [described in @logan2025flexforpcj] because these factors of the external environment will affect where grackles choose to move or spend time [@nathan2008movement]. 

Lastly, conspecific density has also been shown to affect home range size in other bird species [@flockhart2016factors; @garabedian2018relative]. To control for the possibility that home range size may vary between our populations due to conspecific density rather than exploratory traits, we conducted point count surveys to measure grackle population density. We placed 225 point count stations across the landscape encompassing each population (Tempe, AZ; Sacramento, CA). For each study population, the first central point was randomly placed within 500 m of the center of the study area. The remaining points were placed in a 500 m grid pattern extending out from this central point. In total, the sample area was 7 × 7 km. Each point was visited once during the non-breeding season (Sep-Mar). During the survey, researchers recorded all grackles visually and aurally detected for six minutes. We will quantified region-specific grackle density by fitting a hierarchical model that accounts for imperfect detection. Specifically, we used the model developed by @amundson2014hierarchical which integrates data on time of detection and distance estimates to account for the probability a bird is available to be detected (pa), and the probability it is detected given it is available (pd), respectively [@nichols2009inferences]. All parameters (density, pa, and pd) were modeled as a function of the region. Because we expected that vocalization rates will be greater for males than females, we will also modeled pa as a function of sex. We extracted the estimate of expected point-level density for each region and use this estimate as the covariate in our movement models.
...
#### Descriptives ####
describe(AZpts$count) # GPS pts per bird mean = 848, range = 42-1781
describe(CApts$count) # mean = 1199, range = 47-1968
AZpts$day = 1
...
## HYPOTHESIS 1: Individual differences in exploration are related to variation in space use.
...
```
Figure X: Average and standard deviation of the effect of each covariate on step length per time. Data from grackles in Arizona is shown in green and data from California grackles is shown in yellow.
...
```
Figure X: Results for the analysis of spatial location preference, measured as autocorrelation of grackle locations across each study site. The left plot shows the posterior mean estimates (dots) for the relationship of each predictor variable with autocorrelation of space use. Significant predictor relationships are indicated by lack of overlap of the confidence interval (horizontal lines) with zero (the vertical dashed line). Where applicable, the baseline level of the predictor variable is listed in parentheses along the y-axis. The right plot shows individual variation in autocorrelation. Several individuals show very large confidence intervals because they exhibited rare, long-distance movements.

We investigated which predictors related to the consistency that individuals were found in the same locations across time. At the Arizona site, we see a reliably negative effect of the number of food sites on temporal autocorrelation (Figure X; Table X). Grackles that had a higher number of human food source areas (dumpsters, cat food bowls, outdoor restaurant seating areas, etc.) within their home ranges showed less temporal autocorrelation in space use. At the California site, the confidence intervals of all predictor variables overlapped zero, indicating that we are unable to draw firm conclusions about the relationships with space use (Table X). There was substantial inter-bird variation in autocorrelation of space use at both the California and Arizona sites. 


## HYPOTHESIS 2: Space use behavior will vary among grackles from our two study populations located along different points in the geographic range
...
```
We estimated the great-tailed grackle population density in Tempe, Arizona at 0.073 birds/ha (CI = 0.057-0.093) and in Sacramento, California at 0.016 birds/ha (CI = 0.012-0.021) based on 225 point count stations in Sacramento and 214 in Tempe. Population density was therefore significantly higher at our Arizona site relative to the California site (ß = -1.54, CI = -1.91- -1.17, p < 0.01). Because of the significant correlation between Site ID and population density, we deviated from our preregistered analysis and did not include population density as an independent variable in the space use models, below.


### Home range size
Note that we are not including population density as preregistered because it is significantly correlated with Site.
...
### Effect of captivity on social behavior
