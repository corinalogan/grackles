...
########################################## Run space-use analysis for AZ birds
 ...
 d_ind = read.csv("GracklePtsAviaryByWeek.csv")
 d_ind$Season2 = ifelse(d_ind$Season_ratio > 0.5, "Breeding", "Non-breeding") # separating based on when most of the points were collected.
 smi <- read.csv(url("https://raw.githubusercontent.com/corinalogan/grackles/refs/heads/master/Files/Preregistrations/smiData.csv"), header = T, sep = ",", stringsAsFactors = F)
 colnames(smi)[2] = "Bird.Name"
 d_ind = merge(d_ind, smi[,c(2:4)], by = c("Bird.Name","Site"))
 #library(grackleator) # necessary functions from this package are individually in Helper_Functions.R
 
 source("Code/Process_Arizona_Space_Use_Data.R")
 source("Code/Process_Arizona_Data_For_Stan.R")
 source("Code/Process_Arizona_Data_With_Stan.R") # this takes an extremely long time - 9 covariate models at ~6hrs runtime each
 source("Code/Process_Arizona_Save_Stan_Results.R")

########################################## Run space-use analysis for CA birds
 setwd("~/Documents/Grackle project/Space use/Space Use Workflow")
 d = read.csv("CAgracklePtsAviaryByWeek_new.csv")

 source("Code/Process_California_Space_Use_Data.R")
 source("Code/Process_California_Data_For_Stan.R")
 source("Code/Process_California_Data_With_Stan.R")
 source("Code/Process_California_Save_Stan_Results.R")
...
# give birds back their names for merge
