...
hr_red = hr_red[-which(is.na(hr_red$SMI)),] 
hr_red = hr_red[-which(is.na(hr_red$Group.Size)),] #42 grackles with all data
...
tmp = HR[,-c(1:3,7,13:17)] # only numeric variables
...
# The two exploration variables are substantially correlated (r = -0.57), so only include one in the model at a time

##### DATA CHECKING sites combined #####
library(DHARMa)
library(lme4)
simulationOutput <- simulateResiduals(fittedModel = lm(log(hr) ~ DurationNear + Site + Sex + Season2 + Breeding.Sites + Food.Sites + Group.Size + History + SMI, data=HR), n=250) 
...
library(MCMCglmm)
m1.lat.red = MCMCglmm(log(hr) ~ scale(LatencyFirstLand, center = TRUE, scale = TRUE) + Site +
...
autocorr(m1.lat.red$Sol) #Did fixed effects converge? Values after Lag 0 should be < 0.1
autocorr(m1.lat.red$VCV) #Did error converge? Yes
summary(m1.lat.red) # CA grackles had larger home range sizes

m1.dur.red = MCMCglmm(log(hr) ~ scale(DurationNear, center = TRUE, scale = TRUE) + Site +
                      Sex + Season2 + Breeding.Sites + Food.Sites + Group.Size + History + SMI, 
                      family="gaussian", data=HR, verbose=F, nitt=200000, thin=50, burnin=8000)
autocorr(m1.dur.red$Sol) #Did fixed effects converge? Values after Lag 0 should be < 0.1
autocorr(m1.dur.red$VCV) #Did error converge? Yes
summary(m1.dur.red) # CA grackles had larger home range sizes

```


### Autocorrelation of step length
```{r POST Step length and duration, echo=FALSE, message=FALSE, warning=FALSE}
...
########################################## Run search velocity analysis for AZ birds
 ...
 d_ind = d_ind[,-3] # remove latency, just looking at duration
...
# run once for duration and once for latency
...
d_all.ca$Site = "CA"
d_all = rbind(d_all.az[,c(1:13)],d_all.ca) # combine sites, excluding manip column
...
#### Duration plot ####
res_tot2 = res_tot[-which(res_tot$Site == "Combined"),]
res_tot2$Pred = factor(res_tot2$Pred, levels = c("Duration","Season2","Food.Sites","Breeding.Sites",
                                               "Group.Size","History","Male","SMI"),
                      labels = c("Exploration (Duration near)","Season (Breeding)",
...
# excluding site comparison
res_out_preds_dur = ggplot(res_tot2, aes(Pred, M)) +
  ...
  xlab("Predictor variable") + ylab("Slope coefficient") + coord_flip() + 
  theme(legend.position="bottom") + theme(text=element_text(size=18))

#### Latency plot ####
res_tot2 = res_tot[-which(res_tot$Site == "Combined"),]
res_tot2$Pred = factor(res_tot2$Pred, levels = c("Latency","Season2","Food.Sites","Breeding.Sites",
                                               "Group.Size","History","Male","SMI"),
                      labels = c("Exploration (Latency to approach)","Season (Breeding)",
                                 "Food sites","Breeding sites",
                                 "Group size","Days in aviary","Sex (Female)",
                                 "Body condition"))
res_tot2$Outcome = factor(res_tot2$Outcome, levels = c("MeanLFV","SigmaLFV"),
                         labels = c("Average","Standard Deviation"))
# excluding site comparison
res_out_preds_lat = ggplot(res_tot2, aes(Pred, M)) +
  facet_wrap(~Outcome, scales = "free_x") +
  geom_hline(yintercept=0,linetype="dashed")+
  geom_linerange(
    aes(x = Pred, ymin = L, ymax = H, group = Site, color = Site), linewidth = 2, alpha=0.5,
...
ggarrange(res_out_preds_dur, res_out_preds_lat, nrow = 2, common.legend = T)
### Or maybe I should just include them in the same plot? The other covariates look identical
```
...
```{r POST Step length and latency, echo=FALSE, message=FALSE, warning=FALSE}

############################################################################## Part 1 (Fast)
########################################## Run search velocity analysis for AZ birds
 d_az = read.csv("~/Documents/Grackle project/Space use/Space Use Workflow/AZgracklePtsAviaryByWeek_new.csv")
 d_ind = read.csv("~/Documents/Grackle project/Space use/Space Use Workflow/GracklePtsAviaryByWeek.csv")
 d_ind = d_ind[,-4] # remove duration, just looking at latency
 hist(d_ind$Season_ratio) # A few grackles with points from both breeding & non-breeding seasons, but most are one or the other
 d_ind$Season2 = ifelse(d_ind$Season_ratio > 0.5, "Breeding", "Non-breeding") # separating based on when most of the points were collected.
...
summary(lm(MeanLFV~manip,data=d_all.az)) # no effect of flexibility manipulation, so can leave this out
summary(lm(SigmaLFV~manip,data=d_all.az))
d_all.az = d_all.az[-which(d_all.az$Bird.Name == "Concha" | d_all.az$Bird.Name == "Pernil" | 
                             d_all.az$Bird.Name == "Pupusa" | d_all.az$Bird.Name == "Guacamole"),] # these grackles are included but did not go through exploration

d_all.ca$Site = "CA"
d_all = rbind(d_all.az[,c(1:13)],d_all.ca)
...
```{r POST Step length results table, echo=FALSE, message=FALSE, warning=FALSE}
#####-------Summary table--------#####
## AZ
tab = data.frame(Predictor="Vars", Estimate=1.1, SE=1.1, t.value = 1.1, p.value=1.1)
tab[1,] = c("Novel environment exploration",summary(lm(MeanLFV~Duration,data=d_all.az))$coefficients[2,])
tab[2,] = c("Sex", summary(lm(MeanLFV~Sex,data=d_all.az))$coefficients[2,])
tab[3,] = c("Aviary", summary(lm(MeanLFV~History,data=d_all.az))$coefficients[2,])
tab[4,] = c("Breeding sites", summary(lm(MeanLFV~Breeding.Sites,data=d_all.az))$coefficients[2,])
tab[5,] = c("Food sites", summary(lm(MeanLFV~Food.Sites,data=d_all.az))$coefficients[2,])
tab[6,] = c("Body condition", summary(lm(MeanLFV~SMI,data=d_all.az))$coefficients[2,])
tab[7,] = c("Group size", summary(lm(MeanLFV~Group.Size,data=d_all.az))$coefficients[2,])
tab[8,] = c("Non-breeding season", summary(lm(MeanLFV~NonBreeding,data=d_all.az))$coefficients[2,])
tab[9,] = c("Breeding season", summary(lm(MeanLFV~Breeding,data=d_all.az))$coefficients[2,])

tab$Group = "Mean step length per time"

tab[10,] = c("Novel environment exploration",summary(lm(SigmaLFV~Duration,data=d_all.az))$coefficients[2,],"Standard deviation step length per time")
tab[11,] = c("Sex", summary(lm(SigmaLFV~Sex,data=d_all.az))$coefficients[2,],"Standard deviation step length per time")
tab[12,] = c("Aviary", summary(lm(SigmaLFV~History,data=d_all.az))$coefficients[2,],"Standard deviation step length per time")
tab[13,] = c("Breeding sites", summary(lm(SigmaLFV~Breeding.Sites,data=d_all.az))$coefficients[2,],"Standard deviation step length per time")
tab[14,] = c("Food sites", summary(lm(SigmaLFV~Food.Sites,data=d_all.az))$coefficients[2,],"Standard deviation step length per time")
tab[15,] = c("Body condition", summary(lm(SigmaLFV~SMI,data=d_all.az))$coefficients[2,],"Standard deviation step length per time")
tab[16,] = c("Group size", summary(lm(SigmaLFV~Group.Size,data=d_all.az))$coefficients[2,],"Standard deviation step length per time")
tab[17,] = c("Non-breeding season", summary(lm(SigmaLFV~NonBreeding,data=d_all.az))$coefficients[2,],"Standard deviation step length per time")
tab[18,] = c("Breeding season", summary(lm(SigmaLFV~Breeding,data=d_all.az))$coefficients[2,],"Standard deviation step length per time")

tab$Estimate = round(as.numeric(tab$Estimate),digits = 2)
tab$SE = round(as.numeric(tab$SE),digits = 2)
tab$t.value = round(as.numeric(tab$t.value),digits = 2)
tab$p.value = round(as.numeric(tab$p.value),digits = 2)

library(insight)
tab %>%
  format_table(stars = T) %>%
  export_table(tab,format = "html")

#write.csv(tab, "Table1az.csv")


## CA
tab = data.frame(Predictor="Vars", Estimate=1.1, SE=1.1, t.value = 1.1, p.value=1.1)
tab[1,] = c("Novel environment exploration",summary(lm(MeanLFV~Duration,data=d_all.ca))$coefficients[2,])
tab[2,] = c("Sex", summary(lm(MeanLFV~Sex,data=d_all.ca))$coefficients[2,])
tab[3,] = c("Aviary", summary(lm(MeanLFV~History,data=d_all.ca))$coefficients[2,])
tab[4,] = c("Breeding sites", summary(lm(MeanLFV~Breeding.Sites,data=d_all.ca))$coefficients[2,])
tab[5,] = c("Food sites", summary(lm(MeanLFV~Food.Sites,data=d_all.ca))$coefficients[2,])
tab[6,] = c("Body condition", summary(lm(MeanLFV~SMI,data=d_all.ca))$coefficients[2,])
tab[7,] = c("Group size", summary(lm(MeanLFV~Group.Size,data=d_all.ca))$coefficients[2,])
tab[8,] = c("Non-breeding season", summary(lm(MeanLFV~NonBreeding,data=d_all.ca))$coefficients[2,])
tab[9,] = c("Breeding season", summary(lm(MeanLFV~Breeding,data=d_all.ca))$coefficients[2,])

tab$Group = "Mean step length per time"

tab[10,] = c("Novel environment exploration",summary(lm(SigmaLFV~Duration,data=d_all.ca))$coefficients[2,],"Standard deviation step length per time")
tab[11,] = c("Sex", summary(lm(SigmaLFV~Sex,data=d_all.ca))$coefficients[2,],"Standard deviation step length per time")
tab[12,] = c("Aviary", summary(lm(SigmaLFV~History,data=d_all.ca))$coefficients[2,],"Standard deviation step length per time")
tab[13,] = c("Breeding sites", summary(lm(SigmaLFV~Breeding.Sites,data=d_all.ca))$coefficients[2,],"Standard deviation step length per time")
tab[14,] = c("Food sites", summary(lm(SigmaLFV~Food.Sites,data=d_all.ca))$coefficients[2,],"Standard deviation step length per time")
tab[15,] = c("Body condition", summary(lm(SigmaLFV~SMI,data=d_all.ca))$coefficients[2,],"Standard deviation step length per time")
tab[16,] = c("Group size", summary(lm(SigmaLFV~Group.Size,data=d_all.ca))$coefficients[2,],"Standard deviation step length per time")
tab[17,] = c("Non-breeding season", summary(lm(SigmaLFV~NonBreeding,data=d_all.ca))$coefficients[2,],"Standard deviation step length per time")
tab[18,] = c("Breeding season", summary(lm(SigmaLFV~Breeding,data=d_all.ca))$coefficients[2,],"Standard deviation step length per time")
...
#####-------Exploration by site--------#####
summary(lm(Duration ~ Site + manip, data = d_all))
ggplot(d_all, aes(Site, Duration)) + 
  geom_boxplot(aes(fill = Site)) +
  theme_bw()+
  scale_fill_manual(values = c("darkcyan", "darkgoldenrod2")) +
  xlab("Site") + ylab("Novel environment exploration") + 
