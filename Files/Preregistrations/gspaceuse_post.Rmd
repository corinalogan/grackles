...
tmp = HR[,-c(1,2,3,7,11,13)] # only numeric variables
...
# Also, Group size is correlated with Food Sites; and History is correlated with Food sites and breeding sites
summary(lm(log(hr) ~ Group.Size + Site, data = HR))
summary(lm(log(hr) ~ History + Site, data = HR))
# Food sites correlated with HR, so leave that in model as it has potential to influence results. Other variables (group size & history) do not influence results


##### DATA CHECKING sites combined #####
library(DHARMa)
library(lme4)
simulationOutput <- simulateResiduals(fittedModel = lm(log(hr) ~ DurationNear + Sex + Season + Breeding.Sites + Food.Sites, data=HR), n=250) # excluding Food.sites because of correlatio with History & Group size
...
### Autocorrelation of step length
```{r POST Step length and exploration, eval=FALSE, include=FALSE}
...
d_all.az$Site = "AZ"
d_all.ca$Site = "CA"
d_all = rbind(d_all.az,d_all.ca)
d_all$manip = ifelse(d_all$Bird.Name == "Adobo" | d_all$Bird.Name == "Burrito" | d_all$Bird.Name == "Chalupa" | 
                       d_all$Bird.Name == "Mole" | d_all$Bird.Name == "Diablo" | d_all$Bird.Name == "Chilaquile" | 
                       d_all$Bird.Name == "Pollito",
                     "yes","no")
summary(lm(MeanLFV~Site+manip,data=d_all))
summary(lm(SigmaLFV~Site+manip,data=d_all))
...
res_tot2$Pred = factor(res_tot2$Pred, levels = c("Duration","Breeding","NonBreeding","Food.Sites","Breeding.Sites",
                                               "Group.Size","History","Male","SMI"),
                      labels = c("Novel environment exploration","Breeding season","Non-breeding season",
