...
#remove Guacamole & Memela who did not pass the reversal experiment and therefore was not offered the MAB experiments
d <- d[!d$ID=="Guacamole" & !d$ID=="Memela",]
#remove Chalupa who did not solve any MAB loci and so we can't measure solution switching
d <- d[!d$ID=="Chalupa",]
#remove pilot birds
d <- d[!d$ID == "Fajita" & !d$ID == "Empanada", ]
...
# Data checking - Which distribution should we use? 
library(DHARMa)
library(lme4)
...
# Need to put latency on same scale (trials to reverse vs seconds to switch)
# cntx.rpt = rpt(log(Value) ~ Test + ReverseNumber + (1|ID), grname = "ID", data = d, datatype = "Gaussian", nboot = 5000, npermut = 100)
# summary(cntx.rpt) # R = 0.12, p = 0.03
# 
# rm <- lmer(log(Value) ~ Test + ReverseNumber + (1|ID), 
# 	    data = d)
# plot(rm)
# 
# boxplot(residuals(rm) ~ d$Test)
...
mean(repeata) #0.13
var(repeata) #0.02 variance
posterior.mode(repeata) #5.21e-5
...
for(i in 1:1000){
  ...
  tmp = rbind(tmp1,tmp2,tmp3,tmp4,tmp5,tmp6,tmp7,tmp8)
  m <- MCMCglmm(log(TrialsToReverse) ~ ReverseNumber, random=~ID, family="gaussian", data=tmp, verbose=F, prior=prior, nitt=100000, thin=500, burnin=500)
  rpt <- m$VCV[,"ID"]/(m$VCV[,"ID"]+m$VCV[,"units"]) #latent scale adjusted repeatability and its credible interval
  results[i] = mean(rpt)
}
hist(results)
...
#p = 0.004 - Our repeatability value of 0.13 is significantly greater than that expected if birds are performing randomly in each reversal
