...
library(DHARMa)
library(lme4)
rev = read.csv("g_flexmanip2rep_data_xcontext.csv") # data file with reversal learning measure in seconds to reverse
simulationOutput <- simulateResiduals(fittedModel = glmer(log(Dur.sec) ~
    Test + cent.rev + (1 | ID), family = gaussian, data = rev), n = 250)  #250 simulations, but if want higher precision change n>1000
...
HPDinterval(repeata, 0.95) #0.11-0.57
